...
from . import PaymentTransaction178
from . import StatusReasonInformation14

class OriginalPaymentInstruction56(base_types._BaseFieldType):
	...
	def TxInfAndSts(self, value):
		self._TxInfAndSts = value if value is not None else base_types.UninitialisedField(self, 'TxInfAndSts', PaymentTransaction178, True)

	@TxInfAndSts.deleter
	def TxInfAndSts(self):
		del self._TxInfAndSts
		self._TxInfAndSts = base_types.UninitialisedField(self, 'TxInfAndSts', PaymentTransaction178, True)
	...
	_field_defs = frozenset((
		...
		base_types.FieldEntry(name='TxInfAndSts', type=PaymentTransaction178, min=0, max=None, mutex_group=None, array=True),
	))