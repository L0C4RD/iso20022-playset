...
from . import OvernightIndexSwapTransaction4
...
class OvernightIndexSwap4Choice(base_types._BaseFieldType):
	...
	@Tx.setter
	def Tx(self, value):
		self._Tx = value if value is not None else base_types.UninitialisedField(self, 'Tx', OvernightIndexSwapTransaction4, True)

	@Tx.deleter
	def Tx(self):
		del self._Tx
		self._Tx = base_types.UninitialisedField(self, 'Tx', OvernightIndexSwapTransaction4, True)
