...
class LoanData139(base_types._BaseFieldType):
	...
	def EarlstCallBckDt(self, value):
		self._EarlstCallBckDt = value if value is not None else base_types.UninitialisedField(self, 'EarlstCallBckDt', ISODate, False)

	@EarlstCallBckDt.deleter
	def EarlstCallBckDt(self):
		del self._EarlstCallBckDt
		self._EarlstCallBckDt = base_types.UninitialisedField(self, 'EarlstCallBckDt', ISODate, False)
	...
	def MinNtcePrd(self, value):
		self._MinNtcePrd = value if value is not None else base_types.UninitialisedField(self, 'MinNtcePrd', Max20PositiveNumber, False)

	@MinNtcePrd.deleter
	def MinNtcePrd(self):
		del self._MinNtcePrd
		self._MinNtcePrd = base_types.UninitialisedField(self, 'MinNtcePrd', Max20PositiveNumber, False)
