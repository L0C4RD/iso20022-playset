...
class InvestmentAccountOwnershipInformation16(base_types._BaseFieldType):
	...
	def InvstrPrflVldtn(self, value):
		self._InvstrPrflVldtn = value if value is not None else base_types.UninitialisedField(self, 'InvstrPrflVldtn', PartyProfileInformation5, True)

	@InvstrPrflVldtn.deleter
	def InvstrPrflVldtn(self):
		del self._InvstrPrflVldtn
		self._InvstrPrflVldtn = base_types.UninitialisedField(self, 'InvstrPrflVldtn', PartyProfileInformation5, True)
	...
	@Pty.setter
	def Pty(self, value):
		self._Pty = value if value is not None else base_types.UninitialisedField(self, 'Pty', Party47Choice, False)

	@Pty.deleter
	def Pty(self):
		del self._Pty
		self._Pty = base_types.UninitialisedField(self, 'Pty', Party47Choice, False)
