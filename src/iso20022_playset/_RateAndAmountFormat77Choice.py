...
from . import RateTypeAndPercentageRate22
from . import RateValueType7Code
from . import RestrictedFINActiveCurrencyAnd13DecimalAmount

class RateAndAmountFormat77Choice(base_types._BaseFieldType):
	...
	def RateTpAndRate(self, value):
		self._RateTpAndRate = value if value is not None else base_types.UninitialisedField(self, 'RateTpAndRate', RateTypeAndPercentageRate22, False)

	@RateTpAndRate.deleter
	def RateTpAndRate(self):
		del self._RateTpAndRate
		self._RateTpAndRate = base_types.UninitialisedField(self, 'RateTpAndRate', RateTypeAndPercentageRate22, False)
	...
	_field_defs = frozenset((
		...
		base_types.FieldEntry(name='RateTpAndRate', type=RateTypeAndPercentageRate22, min=0, max=1, mutex_group=1, array=False),
	))