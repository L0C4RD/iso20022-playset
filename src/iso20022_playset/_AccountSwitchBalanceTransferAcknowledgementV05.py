...
class AccountSwitchBalanceTransferAcknowledgementV05(base_types._BaseFieldType):
	...
	__slots__ = ["_AcctSwtchDtls", "_BalTrf", "_MsgId", "_OdAcct", "_OdAcctBal", "_SplmtryData"]
	@property
	...
	def BalTrf(self, value):
		self._BalTrf = value if value is not None else base_types.UninitialisedField(self, 'BalTrf', BalanceTransfer5, True)

	@BalTrf.deleter
	def BalTrf(self):
		del self._BalTrf
		self._BalTrf = base_types.UninitialisedField(self, 'BalTrf', BalanceTransfer5, True)
	...
	def OdAcctBal(self, value):
		self._OdAcctBal = value if value is not None else base_types.UninitialisedField(self, 'OdAcctBal', AmountAndDirection5, False)

	@OdAcctBal.deleter
	def OdAcctBal(self):
		del self._OdAcctBal
		self._OdAcctBal = base_types.UninitialisedField(self, 'OdAcctBal', AmountAndDirection5, False)
