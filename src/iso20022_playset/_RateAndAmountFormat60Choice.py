...
from . import RateTypeAndPercentageRate13
from . import RateValueType7Code

class RateAndAmountFormat60Choice(base_types._BaseFieldType):
	...
	def RateTpAndRate(self, value):
		self._RateTpAndRate = value if value is not None else base_types.UninitialisedField(self, 'RateTpAndRate', RateTypeAndPercentageRate13, False)

	@RateTpAndRate.deleter
	def RateTpAndRate(self):
		del self._RateTpAndRate
		self._RateTpAndRate = base_types.UninitialisedField(self, 'RateTpAndRate', RateTypeAndPercentageRate13, False)
	...
	_field_defs = frozenset((
		...
		base_types.FieldEntry(name='RateTpAndRate', type=RateTypeAndPercentageRate13, min=0, max=1, mutex_group=1, array=False),
	))