...
from . import CreditTransferTransaction74
...
class PaymentInstruction48(base_types._BaseFieldType):
	...
	def CdtTrfTx(self, value):
		self._CdtTrfTx = value if value is not None else base_types.UninitialisedField(self, 'CdtTrfTx', CreditTransferTransaction74, True)

	@CdtTrfTx.deleter
	def CdtTrfTx(self):
		del self._CdtTrfTx
		self._CdtTrfTx = base_types.UninitialisedField(self, 'CdtTrfTx', CreditTransferTransaction74, True)
	...
	_field_defs = frozenset((
		base_types.FieldEntry(name='CdtTrfTx', type=CreditTransferTransaction74, min=1, max=None, mutex_group=None, array=True),
