...
class LinkedMessage5Choice(base_types._BaseFieldType):
	...
	def OthrRef(self, value):
		self._OthrRef = value if value is not None else base_types.UninitialisedField(self, 'OthrRef', AdditionalReference13, False)

	@OthrRef.deleter
	def OthrRef(self):
		del self._OthrRef
		self._OthrRef = base_types.UninitialisedField(self, 'OthrRef', AdditionalReference13, False)
