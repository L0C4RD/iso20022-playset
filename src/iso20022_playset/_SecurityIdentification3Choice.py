...
from . import AlternateSecurityIdentification1
from . import BelgianIdentifier
from . import BloombergIdentifier
...
class SecurityIdentification3Choice(base_types._BaseFieldType):
	...
	def Blmbrg(self, value):
		self._Blmbrg = value if value is not None else base_types.UninitialisedField(self, 'Blmbrg', BloombergIdentifier, False)

	@Blmbrg.deleter
	def Blmbrg(self):
		del self._Blmbrg
		self._Blmbrg = base_types.UninitialisedField(self, 'Blmbrg', BloombergIdentifier, False)
	...
	def OthrPrtryId(self, value):
		self._OthrPrtryId = value if value is not None else base_types.UninitialisedField(self, 'OthrPrtryId', AlternateSecurityIdentification1, False)

	@OthrPrtryId.deleter
	def OthrPrtryId(self):
		del self._OthrPrtryId
		self._OthrPrtryId = base_types.UninitialisedField(self, 'OthrPrtryId', AlternateSecurityIdentification1, False)
	...
	_field_defs = frozenset((
		...
		base_types.FieldEntry(name='Blmbrg', type=BloombergIdentifier, min=0, max=1, mutex_group=1, array=False),
		...
		base_types.FieldEntry(name='OthrPrtryId', type=AlternateSecurityIdentification1, min=0, max=1, mutex_group=1, array=False),
