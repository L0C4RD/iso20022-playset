...
from . import BalanceQuantity9Choice
from . import ShortLong1Code

class Balance6(base_types._BaseFieldType):
	...
	@Qty.setter
	def Qty(self, value):
		self._Qty = value if value is not None else base_types.UninitialisedField(self, 'Qty', BalanceQuantity9Choice, False)

	@Qty.deleter
	def Qty(self):
		del self._Qty
		self._Qty = base_types.UninitialisedField(self, 'Qty', BalanceQuantity9Choice, False)
	...
	_field_defs = frozenset((
		base_types.FieldEntry(name='Qty', type=BalanceQuantity9Choice, min=1, max=1, mutex_group=None, array=False),
