...
class ATMDeviceReportV04(base_types._BaseFieldType):
	...
	def ATMDvcRpt(self, value):
		self._ATMDvcRpt = value if value is not None else base_types.UninitialisedField(self, 'ATMDvcRpt', ATMDeviceReport4, False)

	@ATMDvcRpt.deleter
	def ATMDvcRpt(self):
		del self._ATMDvcRpt
		self._ATMDvcRpt = base_types.UninitialisedField(self, 'ATMDvcRpt', ATMDeviceReport4, False)
