...
from . import PartyAuditTrailSearchCriteria2
...
class PartyAuditTrailQueryV01(base_types._BaseFieldType):
	...
	def SchCrit(self, value):
		self._SchCrit = value if value is not None else base_types.UninitialisedField(self, 'SchCrit', PartyAuditTrailSearchCriteria2, False)

	@SchCrit.deleter
	def SchCrit(self):
		del self._SchCrit
		self._SchCrit = base_types.UninitialisedField(self, 'SchCrit', PartyAuditTrailSearchCriteria2, False)
	...
	_field_defs = frozenset((
		...
		base_types.FieldEntry(name='SchCrit', type=PartyAuditTrailSearchCriteria2, min=1, max=1, mutex_group=None, array=False),
