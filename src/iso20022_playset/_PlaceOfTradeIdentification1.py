...
class PlaceOfTradeIdentification1(base_types._BaseFieldType):
	...
	def MktTpAndId(self, value):
		self._MktTpAndId = value if value is not None else base_types.UninitialisedField(self, 'MktTpAndId', MarketIdentification84, False)

	@MktTpAndId.deleter
	def MktTpAndId(self):
		del self._MktTpAndId
		self._MktTpAndId = base_types.UninitialisedField(self, 'MktTpAndId', MarketIdentification84, False)

	_field_defs = frozenset((
		base_types.FieldEntry(name='LEI', type=LEIIdentifier, min=0, max=1, mutex_group=None, array=False),
		base_types.FieldEntry(name='MktTpAndId', type=MarketIdentification84, min=0, max=1, mutex_group=None, array=False),
	))