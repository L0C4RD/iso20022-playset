...
from . import CardPaymentTransaction145
...
class AcceptorCancellationResponse13(base_types._BaseFieldType):
	...
	def TxRspn(self, value):
		self._TxRspn = value if value is not None else base_types.UninitialisedField(self, 'TxRspn', CardPaymentTransaction145, False)

	@TxRspn.deleter
	def TxRspn(self):
		del self._TxRspn
		self._TxRspn = base_types.UninitialisedField(self, 'TxRspn', CardPaymentTransaction145, False)
