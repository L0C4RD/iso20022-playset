...
from . import BalanceInquiryRequest8
from . import BatchRequest7
from . import CardAcquisitionRequest3
from . import CardPaymentEnvironment81
from . import EnableServiceRequest6
...
class ServiceRequest8(base_types._BaseFieldType):
	...
	def BalNqryReq(self, value):
		self._BalNqryReq = value if value is not None else base_types.UninitialisedField(self, 'BalNqryReq', BalanceInquiryRequest8, False)

	@BalNqryReq.deleter
	def BalNqryReq(self):
		del self._BalNqryReq
		self._BalNqryReq = base_types.UninitialisedField(self, 'BalNqryReq', BalanceInquiryRequest8, False)
	...
	def BtchReq(self, value):
		self._BtchReq = value if value is not None else base_types.UninitialisedField(self, 'BtchReq', BatchRequest7, False)

	@BtchReq.deleter
	def BtchReq(self):
		del self._BtchReq
		self._BtchReq = base_types.UninitialisedField(self, 'BtchReq', BatchRequest7, False)
	...
	def NblSvcReq(self, value):
		self._NblSvcReq = value if value is not None else base_types.UninitialisedField(self, 'NblSvcReq', EnableServiceRequest6, False)

	@NblSvcReq.deleter
	def NblSvcReq(self):
		del self._NblSvcReq
		self._NblSvcReq = base_types.UninitialisedField(self, 'NblSvcReq', EnableServiceRequest6, False)
	...
	def StordValReq(self, value):
		self._StordValReq = value if value is not None else base_types.UninitialisedField(self, 'StordValReq', StoredValueRequest8, False)

	@StordValReq.deleter
	def StordValReq(self):
		del self._StordValReq
		self._StordValReq = base_types.UninitialisedField(self, 'StordValReq', StoredValueRequest8, False)
