...
from . import CardNotReceivedData1
from . import Cardholder23
from . import ContentInformationType41
from . import DestinationData1
from . import EncryptedData2
from . import FraudulentTransactionData4
from . import Header72
from . import IssuerData1
from . import Jurisdiction2
from . import LocalData23
from . import Max70Text
from . import OriginatorData2
from . import ProgrammeMode6
from . import ReceiverData1
from . import Reconciliation5
from . import ReportedFraud6
from . import SenderData1
from . import SettlementService7
from . import Token5

class FraudReportingInitiationV04(base_types._BaseFieldType):

	__slots__ = ["_Acqrr", "_AddtlFee", "_AddtlInf", "_Card", "_CardNotRcvdDtls", "_Crdhldr", "_Dstn", "_FrdTxId", "_FrdlntTxData", "_Hdr", "_Issr", "_Jursdctn", "_LclData", "_NtlData", "_Orgtr", "_Prgrmm", "_PrtctdData", "_PrvtData", "_Rcncltn", "_Rcvr", "_RptdFrd", "_SctyTrlr", "_Sndr", "_SttlmSvc", "_Tkn"]
	@property
	def Acqrr(self):
		return self._Acqrr
	...
	def AddtlInf(self, value):
		self._AddtlInf = value if value is not None else base_types.UninitialisedField(self, 'AddtlInf', ATICALaxProcessing, False)

	@AddtlInf.deleter
	def AddtlInf(self):
		del self._AddtlInf
		self._AddtlInf = base_types.UninitialisedField(self, 'AddtlInf', ATICALaxProcessing, False)
	...
	def CardNotRcvdDtls(self, value):
		self._CardNotRcvdDtls = value if value is not None else base_types.UninitialisedField(self, 'CardNotRcvdDtls', CardNotReceivedData1, False)

	@CardNotRcvdDtls.deleter
	def CardNotRcvdDtls(self):
		del self._CardNotRcvdDtls
		self._CardNotRcvdDtls = base_types.UninitialisedField(self, 'CardNotRcvdDtls', CardNotReceivedData1, False)
	...
	def FrdlntTxData(self, value):
		self._FrdlntTxData = value if value is not None else base_types.UninitialisedField(self, 'FrdlntTxData', FraudulentTransactionData4, False)

	@FrdlntTxData.deleter
	def FrdlntTxData(self):
		del self._FrdlntTxData
		self._FrdlntTxData = base_types.UninitialisedField(self, 'FrdlntTxData', FraudulentTransactionData4, False)
	...
	def LclData(self, value):
		self._LclData = value if value is not None else base_types.UninitialisedField(self, 'LclData', LocalData23, False)

	@LclData.deleter
	def LclData(self):
		del self._LclData
		self._LclData = base_types.UninitialisedField(self, 'LclData', LocalData23, False)
	...
	def RptdFrd(self, value):
		self._RptdFrd = value if value is not None else base_types.UninitialisedField(self, 'RptdFrd', ReportedFraud6, False)

	@RptdFrd.deleter
	def RptdFrd(self):
		del self._RptdFrd
		self._RptdFrd = base_types.UninitialisedField(self, 'RptdFrd', ReportedFraud6, False)
	...
	_field_defs = frozenset((
		...
		base_types.FieldEntry(name='AddtlInf', type=ATICALaxProcessing, min=0, max=1, mutex_group=None, array=False),
		base_types.FieldEntry(name='Card', type=CardData17, min=0, max=1, mutex_group=None, array=False),
		base_types.FieldEntry(name='CardNotRcvdDtls', type=CardNotReceivedData1, min=0, max=1, mutex_group=None, array=False),
		...
		base_types.FieldEntry(name='FrdlntTxData', type=FraudulentTransactionData4, min=1, max=1, mutex_group=None, array=False),
		...
		base_types.FieldEntry(name='LclData', type=LocalData23, min=0, max=1, mutex_group=None, array=False),
		...
		base_types.FieldEntry(name='RptdFrd', type=ReportedFraud6, min=1, max=1, mutex_group=None, array=False),
