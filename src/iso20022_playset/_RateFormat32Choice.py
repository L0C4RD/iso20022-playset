...
from . import RateType14Code

class RateFormat32Choice(base_types._BaseFieldType):
	...
	def NotSpcfdRate(self, value):
		self._NotSpcfdRate = value if value is not None else base_types.UninitialisedField(self, 'NotSpcfdRate', RateType14Code, False)

	@NotSpcfdRate.deleter
	def NotSpcfdRate(self):
		del self._NotSpcfdRate
		self._NotSpcfdRate = base_types.UninitialisedField(self, 'NotSpcfdRate', RateType14Code, False)
	...
	_field_defs = frozenset((
		base_types.FieldEntry(name='NotSpcfdRate', type=RateType14Code, min=0, max=1, mutex_group=1, array=False),
