...
class ISINQueryCriteria1(base_types._BaseFieldType):
	...
	@Idr.setter
	def Idr(self, value):
		self._Idr = value if value is not None else base_types.UninitialisedField(self, 'Idr', ISINOct2015Identifier, True)

	@Idr.deleter
	def Idr(self):
		del self._Idr
		self._Idr = base_types.UninitialisedField(self, 'Idr', ISINOct2015Identifier, True)
