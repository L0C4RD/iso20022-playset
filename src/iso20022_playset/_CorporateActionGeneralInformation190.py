...
from . import CorporateActionEventType120Choice
from . import FinancialInstrumentQuantity36Choice
from . import RestrictedFINXMax16Text
from . import SecurityIdentification20

class CorporateActionGeneralInformation190(base_types._BaseFieldType):
	...
	def EvtTp(self, value):
		self._EvtTp = value if value is not None else base_types.UninitialisedField(self, 'EvtTp', CorporateActionEventType120Choice, False)

	@EvtTp.deleter
	def EvtTp(self):
		del self._EvtTp
		self._EvtTp = base_types.UninitialisedField(self, 'EvtTp', CorporateActionEventType120Choice, False)
	...
	def FrctnlQty(self, value):
		self._FrctnlQty = value if value is not None else base_types.UninitialisedField(self, 'FrctnlQty', FinancialInstrumentQuantity36Choice, False)

	@FrctnlQty.deleter
	def FrctnlQty(self):
		del self._FrctnlQty
		self._FrctnlQty = base_types.UninitialisedField(self, 'FrctnlQty', FinancialInstrumentQuantity36Choice, False)
	...
	_field_defs = frozenset((
		...
		base_types.FieldEntry(name='EvtTp', type=CorporateActionEventType120Choice, min=1, max=1, mutex_group=None, array=False),
