...
from . import EncryptedContent8
from . import Number
from . import Recipient7Choice
...
class EnvelopedData12(base_types._BaseFieldType):
	...
	def NcrptdCntt(self, value):
		self._NcrptdCntt = value if value is not None else base_types.UninitialisedField(self, 'NcrptdCntt', EncryptedContent8, False)

	@NcrptdCntt.deleter
	def NcrptdCntt(self):
		del self._NcrptdCntt
		self._NcrptdCntt = base_types.UninitialisedField(self, 'NcrptdCntt', EncryptedContent8, False)
	...
	@Rcpt.setter
	def Rcpt(self, value):
		self._Rcpt = value if value is not None else base_types.UninitialisedField(self, 'Rcpt', Recipient7Choice, True)

	@Rcpt.deleter
	def Rcpt(self):
		del self._Rcpt
		self._Rcpt = base_types.UninitialisedField(self, 'Rcpt', Recipient7Choice, True)
