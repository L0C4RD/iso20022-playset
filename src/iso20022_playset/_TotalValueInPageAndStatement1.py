...
class TotalValueInPageAndStatement1(base_types._BaseFieldType):
	...
	def TtlElgblCollVal(self, value):
		self._TtlElgblCollVal = value if value is not None else base_types.UninitialisedField(self, 'TtlElgblCollVal', AmountAndDirection6, False)

	@TtlElgblCollVal.deleter
	def TtlElgblCollVal(self):
		del self._TtlElgblCollVal
		self._TtlElgblCollVal = base_types.UninitialisedField(self, 'TtlElgblCollVal', AmountAndDirection6, False)
