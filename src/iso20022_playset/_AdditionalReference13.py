...
class AdditionalReference13(base_types._BaseFieldType):
	...
	def RefIssr(self, value):
		self._RefIssr = value if value is not None else base_types.UninitialisedField(self, 'RefIssr', PartyIdentification125Choice, False)

	@RefIssr.deleter
	def RefIssr(self):
		del self._RefIssr
		self._RefIssr = base_types.UninitialisedField(self, 'RefIssr', PartyIdentification125Choice, False)
	...
	_field_defs = frozenset((
		...
		base_types.FieldEntry(name='RefIssr', type=PartyIdentification125Choice, min=0, max=1, mutex_group=None, array=False),
	))