...
from . import CardAccountBalance1
from . import ISO8583AccountTypeCode

class AccountBalance4(base_types._BaseFieldType):
	...
	@Bal.setter
	def Bal(self, value):
		self._Bal = value if value is not None else base_types.UninitialisedField(self, 'Bal', CardAccountBalance1, True)

	@Bal.deleter
	def Bal(self):
		del self._Bal
		self._Bal = base_types.UninitialisedField(self, 'Bal', CardAccountBalance1, True)

	_field_defs = frozenset((
		base_types.FieldEntry(name='AcctTp', type=ISO8583AccountTypeCode, min=1, max=1, mutex_group=None, array=False),
		base_types.FieldEntry(name='Bal', type=CardAccountBalance1, min=1, max=None, mutex_group=None, array=True),
	))