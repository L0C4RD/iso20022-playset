...
from . import CancellationReason27
from . import FXCancellation4Choice
from . import PartyIdentification156
from . import References60Choice
from . import RestrictedFINXMax16Text
from . import SecuritiesAccount30
from . import SupplementaryData1
from . import TransactionDetails160
...
class SecuritiesTransactionCancellationRequest002V07(base_types._BaseFieldType):
	...
	def AcctOwnrTxId(self, value):
		self._AcctOwnrTxId = value if value is not None else base_types.UninitialisedField(self, 'AcctOwnrTxId', References60Choice, False)

	@AcctOwnrTxId.deleter
	def AcctOwnrTxId(self):
		del self._AcctOwnrTxId
		self._AcctOwnrTxId = base_types.UninitialisedField(self, 'AcctOwnrTxId', References60Choice, False)
	...
	def CxlRsn(self, value):
		self._CxlRsn = value if value is not None else base_types.UninitialisedField(self, 'CxlRsn', CancellationReason27, False)

	@CxlRsn.deleter
	def CxlRsn(self):
		del self._CxlRsn
		self._CxlRsn = base_types.UninitialisedField(self, 'CxlRsn', CancellationReason27, False)
	...
	def FxCxl(self, value):
		self._FxCxl = value if value is not None else base_types.UninitialisedField(self, 'FxCxl', FXCancellation4Choice, False)

	@FxCxl.deleter
	def FxCxl(self):
		del self._FxCxl
		self._FxCxl = base_types.UninitialisedField(self, 'FxCxl', FXCancellation4Choice, False)
	...
	def TxDtls(self, value):
		self._TxDtls = value if value is not None else base_types.UninitialisedField(self, 'TxDtls', TransactionDetails160, False)

	@TxDtls.deleter
	def TxDtls(self):
		del self._TxDtls
		self._TxDtls = base_types.UninitialisedField(self, 'TxDtls', TransactionDetails160, False)
