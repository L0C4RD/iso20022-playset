...
class PaymentRequest8(base_types._BaseFieldType):
	...
	def PmtTx(self, value):
		self._PmtTx = value if value is not None else base_types.UninitialisedField(self, 'PmtTx', PaymentTransaction183, False)

	@PmtTx.deleter
	def PmtTx(self):
		del self._PmtTx
		self._PmtTx = base_types.UninitialisedField(self, 'PmtTx', PaymentTransaction183, False)
