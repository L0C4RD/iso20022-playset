...
from . import RateType82Choice
from . import RestrictedFINActiveCurrencyAnd13DecimalAmount

class RateTypeAndAmountAndStatus61(base_types._BaseFieldType):
	...
	def RateTp(self, value):
		self._RateTp = value if value is not None else base_types.UninitialisedField(self, 'RateTp', RateType82Choice, False)

	@RateTp.deleter
	def RateTp(self):
		del self._RateTp
		self._RateTp = base_types.UninitialisedField(self, 'RateTp', RateType82Choice, False)
	...
	_field_defs = frozenset((
		...
		base_types.FieldEntry(name='RateTp', type=RateType82Choice, min=1, max=1, mutex_group=None, array=False),
	))