...
from . import MessageFunction46Code
from . import Number
from . import Traceability8

class Header70(base_types._BaseFieldType):

	__slots__ = ["_CreDtTm", "_InitgPty", "_MsgFctn", "_PrtcolVrsn", "_RcptPty", "_ReTrnsmssnCntr", "_Tracblt", "_XchgId"]
	@property
	def CreDtTm(self):
	...
	def MsgFctn(self, value):
		self._MsgFctn = value if value is not None else base_types.UninitialisedField(self, 'MsgFctn', MessageFunction46Code, False)

	@MsgFctn.deleter
	def MsgFctn(self):
		del self._MsgFctn
		self._MsgFctn = base_types.UninitialisedField(self, 'MsgFctn', MessageFunction46Code, False)
	...
	_field_defs = frozenset((
		...
		base_types.FieldEntry(name='MsgFctn', type=MessageFunction46Code, min=1, max=1, mutex_group=None, array=False),
