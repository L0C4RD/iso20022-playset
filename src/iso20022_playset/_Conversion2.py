...
from . import FinancialInstrumentIdentification1
from . import Unit13
...
class Conversion2(base_types._BaseFieldType):
	...
	def SrcScty(self, value):
		self._SrcScty = value if value is not None else base_types.UninitialisedField(self, 'SrcScty', FinancialInstrumentIdentification1, False)

	@SrcScty.deleter
	def SrcScty(self):
		del self._SrcScty
		self._SrcScty = base_types.UninitialisedField(self, 'SrcScty', FinancialInstrumentIdentification1, False)
	...
	def UnitsDtls(self, value):
		self._UnitsDtls = value if value is not None else base_types.UninitialisedField(self, 'UnitsDtls', Unit13, True)

	@UnitsDtls.deleter
	def UnitsDtls(self):
		del self._UnitsDtls
		self._UnitsDtls = base_types.UninitialisedField(self, 'UnitsDtls', Unit13, True)
