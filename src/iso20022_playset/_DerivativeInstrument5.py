...
from . import AssetClass2
from . import FinancialInstrumentIdentification5Choice
from . import ISODate
...
class DerivativeInstrument5(base_types._BaseFieldType):
	...
	def AsstClssSpcfcAttrbts(self, value):
		self._AsstClssSpcfcAttrbts = value if value is not None else base_types.UninitialisedField(self, 'AsstClssSpcfcAttrbts', AssetClass2, False)

	@AsstClssSpcfcAttrbts.deleter
	def AsstClssSpcfcAttrbts(self):
		del self._AsstClssSpcfcAttrbts
		self._AsstClssSpcfcAttrbts = base_types.UninitialisedField(self, 'AsstClssSpcfcAttrbts', AssetClass2, False)
	...
	def UndrlygInstrm(self, value):
		self._UndrlygInstrm = value if value is not None else base_types.UninitialisedField(self, 'UndrlygInstrm', FinancialInstrumentIdentification5Choice, False)

	@UndrlygInstrm.deleter
	def UndrlygInstrm(self):
		del self._UndrlygInstrm
		self._UndrlygInstrm = base_types.UninitialisedField(self, 'UndrlygInstrm', FinancialInstrumentIdentification5Choice, False)
