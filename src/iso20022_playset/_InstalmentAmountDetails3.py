...
from . import InstalmentAmountDetailsType3Code
...
class InstalmentAmountDetails3(base_types._BaseFieldType):
	...
	@Tp.setter
	def Tp(self, value):
		self._Tp = value if value is not None else base_types.UninitialisedField(self, 'Tp', InstalmentAmountDetailsType3Code, False)

	@Tp.deleter
	def Tp(self):
		del self._Tp
		self._Tp = base_types.UninitialisedField(self, 'Tp', InstalmentAmountDetailsType3Code, False)
	...
	_field_defs = frozenset((
		...
		base_types.FieldEntry(name='Tp', type=InstalmentAmountDetailsType3Code, min=0, max=1, mutex_group=None, array=False),
	))