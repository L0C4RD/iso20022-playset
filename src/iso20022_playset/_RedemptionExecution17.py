...
from . import FundSettlementParameters19
from . import HoldBackInformation2
from . import ISODate
...
from . import PaymentTransaction151
...
from . import TotalFeesAndTaxes42
from . import TransactionChannelType1Choice
from . import UKTaxGroupUnit1Code
from . import UnitPrice22
from . import YesNoIndicator

class RedemptionExecution17(base_types._BaseFieldType):

	__slots__ = ["_BestExctn", "_ClntRef", "_CshSttlmDt", "_CshSttlmDtls", "_CstmrCndctClssfctn", "_CumDvddInd", "_DealRef", "_DealgPricDtls", "_Equlstn", "_FXDtls", "_FinAdvc", "_FinInstrmDtls", "_Grp1Or2Units", "_GrssAmt", "_GtgOrHldBckDtls", "_HldgsRedRate", "_IncmPref", "_InftvPricDtls", "_InftvTaxDtls", "_IntrmPrftAmt", "_LateRpt", "_NetAmt", "_NgtdTrad", "_NonStdSttlmInf", "_OrdrRef", "_OrdrTp", "_OrdrWvrDtls", "_PhysDlvryDtls", "_PhysDlvryInd", "_PrtlRedWhldgAmt", "_PrtlSttlmOfCsh", "_PrtlSttlmOfUnits", "_PrtlyExctdInd", "_ReqdNAVCcy", "_ReqdSttlmCcy", "_RltdPtyDtls", "_Rndg", "_SgntrTp", "_StffClntBrkdwn", "_SttlmAmt", "_SttlmAndCtdyDtls", "_SttlmMtd", "_SubAcctForHldg", "_TradDtTm", "_TxChanlTp", "_TxOvrhd", "_UnitsNb"]
	@property
	def BestExctn(self):
	...
	def CshSttlmDtls(self, value):
		self._CshSttlmDtls = value if value is not None else base_types.UninitialisedField(self, 'CshSttlmDtls', PaymentTransaction151, False)

	@CshSttlmDtls.deleter
	def CshSttlmDtls(self):
		del self._CshSttlmDtls
		self._CshSttlmDtls = base_types.UninitialisedField(self, 'CshSttlmDtls', PaymentTransaction151, False)
	...
	def Equlstn(self, value):
		self._Equlstn = value if value is not None else base_types.UninitialisedField(self, 'Equlstn', Equalisation3, False)

	@Equlstn.deleter
	def Equlstn(self):
		del self._Equlstn
		self._Equlstn = base_types.UninitialisedField(self, 'Equlstn', Equalisation3, False)
	...
	def GtgOrHldBckDtls(self, value):
		self._GtgOrHldBckDtls = value if value is not None else base_types.UninitialisedField(self, 'GtgOrHldBckDtls', HoldBackInformation2, False)

	@GtgOrHldBckDtls.deleter
	def GtgOrHldBckDtls(self):
		del self._GtgOrHldBckDtls
		self._GtgOrHldBckDtls = base_types.UninitialisedField(self, 'GtgOrHldBckDtls', HoldBackInformation2, False)
	...
	def SttlmAndCtdyDtls(self, value):
		self._SttlmAndCtdyDtls = value if value is not None else base_types.UninitialisedField(self, 'SttlmAndCtdyDtls', FundSettlementParameters19, False)

	@SttlmAndCtdyDtls.deleter
	def SttlmAndCtdyDtls(self):
		del self._SttlmAndCtdyDtls
		self._SttlmAndCtdyDtls = base_types.UninitialisedField(self, 'SttlmAndCtdyDtls', FundSettlementParameters19, False)
	...
	def TxOvrhd(self, value):
		self._TxOvrhd = value if value is not None else base_types.UninitialisedField(self, 'TxOvrhd', TotalFeesAndTaxes42, False)

	@TxOvrhd.deleter
	def TxOvrhd(self):
		del self._TxOvrhd
		self._TxOvrhd = base_types.UninitialisedField(self, 'TxOvrhd', TotalFeesAndTaxes42, False)
	...
	_field_defs = frozenset((
		...
		base_types.FieldEntry(name='CshSttlmDtls', type=PaymentTransaction151, min=0, max=1, mutex_group=None, array=False),
		...
		base_types.FieldEntry(name='Equlstn', type=Equalisation3, min=0, max=1, mutex_group=None, array=False),
		...
		base_types.FieldEntry(name='SttlmAndCtdyDtls', type=FundSettlementParameters19, min=0, max=1, mutex_group=None, array=False),
