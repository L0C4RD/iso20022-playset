...
from . import SecuritiesTransactionType58Choice

class SettlementOrCorporateActionEvent33Choice(base_types._BaseFieldType):
	...
	def SctiesTxTp(self, value):
		self._SctiesTxTp = value if value is not None else base_types.UninitialisedField(self, 'SctiesTxTp', SecuritiesTransactionType58Choice, False)

	@SctiesTxTp.deleter
	def SctiesTxTp(self):
		del self._SctiesTxTp
		self._SctiesTxTp = base_types.UninitialisedField(self, 'SctiesTxTp', SecuritiesTransactionType58Choice, False)
	...
	_field_defs = frozenset((
		...
		base_types.FieldEntry(name='SctiesTxTp', type=SecuritiesTransactionType58Choice, min=0, max=1, mutex_group=1, array=False),
	))