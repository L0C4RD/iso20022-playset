...
class FinancialInstrumentReportingTradingVolumeCapDataReportV01(base_types._BaseFieldType):
	...
	def VolCapData(self, value):
		self._VolCapData = value if value is not None else base_types.UninitialisedField(self, 'VolCapData', VolumeCapReport1, True)

	@VolCapData.deleter
	def VolCapData(self):
		del self._VolCapData
		self._VolCapData = base_types.UninitialisedField(self, 'VolCapData', VolumeCapReport1, True)
