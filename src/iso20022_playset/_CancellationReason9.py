...
from . import CancellationReason19Choice
from . import Max210Text

class CancellationReason9(base_types._BaseFieldType):
	...
	@Cd.setter
	def Cd(self, value):
		self._Cd = value if value is not None else base_types.UninitialisedField(self, 'Cd', CancellationReason19Choice, False)

	@Cd.deleter
	def Cd(self):
		del self._Cd
		self._Cd = base_types.UninitialisedField(self, 'Cd', CancellationReason19Choice, False)

	_field_defs = frozenset((
		base_types.FieldEntry(name='AddtlRsnInf', type=Max210Text, min=0, max=1, mutex_group=None, array=False),
		base_types.FieldEntry(name='Cd', type=CancellationReason19Choice, min=1, max=1, mutex_group=None, array=False),
	))