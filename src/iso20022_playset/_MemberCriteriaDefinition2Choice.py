...
class MemberCriteriaDefinition2Choice(base_types._BaseFieldType):
	...
	def NewCrit(self, value):
		self._NewCrit = value if value is not None else base_types.UninitialisedField(self, 'NewCrit', MemberCriteria4, False)

	@NewCrit.deleter
	def NewCrit(self):
		del self._NewCrit
		self._NewCrit = base_types.UninitialisedField(self, 'NewCrit', MemberCriteria4, False)
	...
	_field_defs = frozenset((
		base_types.FieldEntry(name='NewCrit', type=MemberCriteria4, min=0, max=1, mutex_group=1, array=False),
