...
from . import RejectionAndRepairReason40Choice
from . import RestrictedFINXMax210Text

class RejectionOrRepairReason40(base_types._BaseFieldType):
	...
	@Cd.setter
	def Cd(self, value):
		self._Cd = value if value is not None else base_types.UninitialisedField(self, 'Cd', RejectionAndRepairReason40Choice, True)

	@Cd.deleter
	def Cd(self):
		del self._Cd
		self._Cd = base_types.UninitialisedField(self, 'Cd', RejectionAndRepairReason40Choice, True)

	_field_defs = frozenset((
		base_types.FieldEntry(name='AddtlRsnInf', type=RestrictedFINXMax210Text, min=0, max=1, mutex_group=None, array=False),
		base_types.FieldEntry(name='Cd', type=RejectionAndRepairReason40Choice, min=0, max=None, mutex_group=None, array=True),
	))