...
class IntraBalanceMovementCancellationRequestStatusAdviceV02(base_types._BaseFieldType):
	...
	@TxId.setter
	def TxId(self, value):
		self._TxId = value if value is not None else base_types.UninitialisedField(self, 'TxId', References14, False)

	@TxId.deleter
	def TxId(self):
		del self._TxId
		self._TxId = base_types.UninitialisedField(self, 'TxId', References14, False)
