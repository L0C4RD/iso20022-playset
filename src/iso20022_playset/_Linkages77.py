...
from . import IdentificationReference16Choice
...
class Linkages77(base_types._BaseFieldType):
	...
	@Ref.setter
	def Ref(self, value):
		self._Ref = value if value is not None else base_types.UninitialisedField(self, 'Ref', IdentificationReference16Choice, False)

	@Ref.deleter
	def Ref(self):
		del self._Ref
		self._Ref = base_types.UninitialisedField(self, 'Ref', IdentificationReference16Choice, False)

	_field_defs = frozenset((
		base_types.FieldEntry(name='MsgNb', type=DocumentNumber17Choice, min=0, max=1, mutex_group=None, array=False),
		base_types.FieldEntry(name='Ref', type=IdentificationReference16Choice, min=1, max=1, mutex_group=None, array=False),
	))