...
from . import References86Choice

class Linkages81(base_types._BaseFieldType):
	...
	@Ref.setter
	def Ref(self, value):
		self._Ref = value if value is not None else base_types.UninitialisedField(self, 'Ref', References86Choice, False)

	@Ref.deleter
	def Ref(self):
		del self._Ref
		self._Ref = base_types.UninitialisedField(self, 'Ref', References86Choice, False)
	...
	_field_defs = frozenset((
		...
		base_types.FieldEntry(name='Ref', type=References86Choice, min=1, max=1, mutex_group=None, array=False),
