...
from . import GenericIdentification189
from . import SafekeepingPlaceTypeAndAnyBICIdentifier4
...
class SafekeepingPlaceFormat43Choice(base_types._BaseFieldType):
	...
	def Prtry(self, value):
		self._Prtry = value if value is not None else base_types.UninitialisedField(self, 'Prtry', GenericIdentification189, False)

	@Prtry.deleter
	def Prtry(self):
		del self._Prtry
		self._Prtry = base_types.UninitialisedField(self, 'Prtry', GenericIdentification189, False)
	...
	def TpAndId(self, value):
		self._TpAndId = value if value is not None else base_types.UninitialisedField(self, 'TpAndId', SafekeepingPlaceTypeAndAnyBICIdentifier4, False)

	@TpAndId.deleter
	def TpAndId(self):
		del self._TpAndId
		self._TpAndId = base_types.UninitialisedField(self, 'TpAndId', SafekeepingPlaceTypeAndAnyBICIdentifier4, False)
