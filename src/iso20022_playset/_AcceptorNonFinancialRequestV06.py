...
from . import NonFinancialRequestComponent6

class AcceptorNonFinancialRequestV06(base_types._BaseFieldType):
	...
	def NonFinReq(self, value):
		self._NonFinReq = value if value is not None else base_types.UninitialisedField(self, 'NonFinReq', NonFinancialRequestComponent6, False)

	@NonFinReq.deleter
	def NonFinReq(self):
		del self._NonFinReq
		self._NonFinReq = base_types.UninitialisedField(self, 'NonFinReq', NonFinancialRequestComponent6, False)
	...
	_field_defs = frozenset((
		...
		base_types.FieldEntry(name='NonFinReq', type=NonFinancialRequestComponent6, min=1, max=1, mutex_group=None, array=False),
