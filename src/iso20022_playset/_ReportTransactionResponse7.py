...
class ReportTransactionResponse7(base_types._BaseFieldType):
	...
	def TxRpt(self, value):
		self._TxRpt = value if value is not None else base_types.UninitialisedField(self, 'TxRpt', ServiceResponse9, True)

	@TxRpt.deleter
	def TxRpt(self):
		del self._TxRpt
		self._TxRpt = base_types.UninitialisedField(self, 'TxRpt', ServiceResponse9, True)
