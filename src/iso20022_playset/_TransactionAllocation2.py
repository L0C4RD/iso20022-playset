...
class TransactionAllocation2(base_types._BaseFieldType):
	...
	def RltdRefs(self, value):
		self._RltdRefs = value if value is not None else base_types.UninitialisedField(self, 'RltdRefs', References80Choice, True)

	@RltdRefs.deleter
	def RltdRefs(self):
		del self._RltdRefs
		self._RltdRefs = base_types.UninitialisedField(self, 'RltdRefs', References80Choice, True)
	...
	_field_defs = frozenset((
		...
		base_types.FieldEntry(name='RltdRefs', type=References80Choice, min=0, max=None, mutex_group=None, array=True),
	))