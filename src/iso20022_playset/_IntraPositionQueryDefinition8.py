...
from . import IntraPositionQueryCriteria8
from . import MovementResponseType1Code

class IntraPositionQueryDefinition8(base_types._BaseFieldType):
	...
	def SchCrit(self, value):
		self._SchCrit = value if value is not None else base_types.UninitialisedField(self, 'SchCrit', IntraPositionQueryCriteria8, False)

	@SchCrit.deleter
	def SchCrit(self):
		del self._SchCrit
		self._SchCrit = base_types.UninitialisedField(self, 'SchCrit', IntraPositionQueryCriteria8, False)

	_field_defs = frozenset((
		base_types.FieldEntry(name='QryTp', type=MovementResponseType1Code, min=1, max=1, mutex_group=None, array=False),
		base_types.FieldEntry(name='SchCrit', type=IntraPositionQueryCriteria8, min=1, max=1, mutex_group=None, array=False),
	))