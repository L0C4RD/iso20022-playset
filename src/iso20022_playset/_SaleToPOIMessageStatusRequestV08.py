...
from . import MessageStatusRequest9

class SaleToPOIMessageStatusRequestV08(base_types._BaseFieldType):
	...
	def StsReq(self, value):
		self._StsReq = value if value is not None else base_types.UninitialisedField(self, 'StsReq', MessageStatusRequest9, False)

	@StsReq.deleter
	def StsReq(self):
		del self._StsReq
		self._StsReq = base_types.UninitialisedField(self, 'StsReq', MessageStatusRequest9, False)

	_field_defs = frozenset((
		base_types.FieldEntry(name='Hdr', type=Header41, min=1, max=1, mutex_group=None, array=False),
		base_types.FieldEntry(name='SctyTrlr', type=ContentInformationType38, min=0, max=1, mutex_group=None, array=False),
		base_types.FieldEntry(name='StsReq', type=MessageStatusRequest9, min=1, max=1, mutex_group=None, array=False),
	))