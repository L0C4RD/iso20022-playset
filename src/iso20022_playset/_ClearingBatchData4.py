...
class ClearingBatchData4(base_types._BaseFieldType):

	__slots__ = ["_AgtFeeAmt", "_AgtFeeCcy", "_AgtFeeCdtDbt", "_Dt", "_IntrchngFeeAmt", "_IntrchngFeeCcy", "_IntrchngFeeCdtDbt", "_Mtd", "_Prty", "_TtlsAmt", "_TtlsCcy", "_TtlsCdtDbt", "_TtlsCnt"]
	@property
	def AgtFeeAmt(self):
	...
	@Mtd.setter
	def Mtd(self, value):
		self._Mtd = value if value is not None else base_types.UninitialisedField(self, 'Mtd', ClearingMethod3Code, False)

	@Mtd.deleter
	def Mtd(self):
		del self._Mtd
		self._Mtd = base_types.UninitialisedField(self, 'Mtd', ClearingMethod3Code, False)
	...
	_field_defs = frozenset((
		...
		base_types.FieldEntry(name='Mtd', type=ClearingMethod3Code, min=1, max=1, mutex_group=None, array=False),
