...
class TradeError9(base_types._BaseFieldType):
	...
	def LnData(self, value):
		self._LnData = value if value is not None else base_types.UninitialisedField(self, 'LnData', LoanData86, False)

	@LnData.deleter
	def LnData(self):
		del self._LnData
		self._LnData = base_types.UninitialisedField(self, 'LnData', LoanData86, False)
