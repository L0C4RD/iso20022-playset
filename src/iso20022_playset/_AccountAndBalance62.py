...
from . import CorporateActionBalanceDetails44
from . import PartyIdentification136Choice
from . import RestrictedFINXMax140Text
from . import RestrictedFINXMax35Text
from . import SafekeepingPlaceFormat48Choice

class AccountAndBalance62(base_types._BaseFieldType):
	...
	@Bal.setter
	def Bal(self, value):
		self._Bal = value if value is not None else base_types.UninitialisedField(self, 'Bal', CorporateActionBalanceDetails44, False)

	@Bal.deleter
	def Bal(self):
		del self._Bal
		self._Bal = base_types.UninitialisedField(self, 'Bal', CorporateActionBalanceDetails44, False)
	...
	def SfkpgPlc(self, value):
		self._SfkpgPlc = value if value is not None else base_types.UninitialisedField(self, 'SfkpgPlc', SafekeepingPlaceFormat48Choice, False)

	@SfkpgPlc.deleter
	def SfkpgPlc(self):
		del self._SfkpgPlc
		self._SfkpgPlc = base_types.UninitialisedField(self, 'SfkpgPlc', SafekeepingPlaceFormat48Choice, False)

	_field_defs = frozenset((
		base_types.FieldEntry(name='AcctOwnr', type=PartyIdentification136Choice, min=0, max=1, mutex_group=None, array=False),
		base_types.FieldEntry(name='Bal', type=CorporateActionBalanceDetails44, min=0, max=1, mutex_group=None, array=False),
		...
		base_types.FieldEntry(name='SfkpgPlc', type=SafekeepingPlaceFormat48Choice, min=0, max=1, mutex_group=None, array=False),
	))