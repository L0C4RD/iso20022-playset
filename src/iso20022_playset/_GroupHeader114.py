...
from . import PaymentInitiationSource1

class GroupHeader114(base_types._BaseFieldType):

	__slots__ = ["_Authstn", "_CreDtTm", "_CtrlSum", "_FwdgAgt", "_InitgPty", "_InitnSrc", "_MsgId", "_NbOfTxs"]
	@property
	def Authstn(self):
	...
	def InitnSrc(self, value):
		self._InitnSrc = value if value is not None else base_types.UninitialisedField(self, 'InitnSrc', PaymentInitiationSource1, False)

	@InitnSrc.deleter
	def InitnSrc(self):
		del self._InitnSrc
		self._InitnSrc = base_types.UninitialisedField(self, 'InitnSrc', PaymentInitiationSource1, False)
