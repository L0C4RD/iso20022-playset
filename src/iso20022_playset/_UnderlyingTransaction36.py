...
from . import PaymentTransaction172

class UnderlyingTransaction36(base_types._BaseFieldType):
	...
	def TxInf(self, value):
		self._TxInf = value if value is not None else base_types.UninitialisedField(self, 'TxInf', PaymentTransaction172, True)

	@TxInf.deleter
	def TxInf(self):
		del self._TxInf
		self._TxInf = base_types.UninitialisedField(self, 'TxInf', PaymentTransaction172, True)

	_field_defs = frozenset((
		base_types.FieldEntry(name='OrgnlGrpInfAndCxl', type=OriginalGroupHeader21, min=0, max=1, mutex_group=None, array=False),
		base_types.FieldEntry(name='TxInf', type=PaymentTransaction172, min=0, max=None, mutex_group=None, array=True),
	))