...
class SaleToPOIReportRequestV07(base_types._BaseFieldType):
	...
	def RptReq(self, value):
		self._RptReq = value if value is not None else base_types.UninitialisedField(self, 'RptReq', ReportRequest8, False)

	@RptReq.deleter
	def RptReq(self):
		del self._RptReq
		self._RptReq = base_types.UninitialisedField(self, 'RptReq', ReportRequest8, False)
