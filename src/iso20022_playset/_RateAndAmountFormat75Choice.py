...
from . import RateTypeAndPercentageRate20
from . import RateValueType7Code
from . import RestrictedFINActiveCurrencyAnd13DecimalAmount

class RateAndAmountFormat75Choice(base_types._BaseFieldType):
	...
	def RateTpAndRate(self, value):
		self._RateTpAndRate = value if value is not None else base_types.UninitialisedField(self, 'RateTpAndRate', RateTypeAndPercentageRate20, False)

	@RateTpAndRate.deleter
	def RateTpAndRate(self):
		del self._RateTpAndRate
		self._RateTpAndRate = base_types.UninitialisedField(self, 'RateTpAndRate', RateTypeAndPercentageRate20, False)
	...
	_field_defs = frozenset((
		...
		base_types.FieldEntry(name='RateTpAndRate', type=RateTypeAndPercentageRate20, min=0, max=1, mutex_group=1, array=False),
	))