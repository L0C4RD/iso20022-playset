...
from . import AlgorithmIdentification13
...
class KEK4(base_types._BaseFieldType):
	...
	def KEKId(self, value):
		self._KEKId = value if value is not None else base_types.UninitialisedField(self, 'KEKId', KEKIdentifier2, False)

	@KEKId.deleter
	def KEKId(self):
		del self._KEKId
		self._KEKId = base_types.UninitialisedField(self, 'KEKId', KEKIdentifier2, False)
	...
	def KeyNcrptnAlgo(self, value):
		self._KeyNcrptnAlgo = value if value is not None else base_types.UninitialisedField(self, 'KeyNcrptnAlgo', AlgorithmIdentification13, False)

	@KeyNcrptnAlgo.deleter
	def KeyNcrptnAlgo(self):
		del self._KeyNcrptnAlgo
		self._KeyNcrptnAlgo = base_types.UninitialisedField(self, 'KeyNcrptnAlgo', AlgorithmIdentification13, False)
