...
from . import PartyOrOperationalError4Choice
...
class PartyReportV02(base_types._BaseFieldType):
	...
	def RptOrErr(self, value):
		self._RptOrErr = value if value is not None else base_types.UninitialisedField(self, 'RptOrErr', PartyOrOperationalError4Choice, False)

	@RptOrErr.deleter
	def RptOrErr(self):
		del self._RptOrErr
		self._RptOrErr = base_types.UninitialisedField(self, 'RptOrErr', PartyOrOperationalError4Choice, False)
