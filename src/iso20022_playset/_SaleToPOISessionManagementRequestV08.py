...
from . import SessionManagementRequest9

class SaleToPOISessionManagementRequestV08(base_types._BaseFieldType):
	...
	def SsnMgmtReq(self, value):
		self._SsnMgmtReq = value if value is not None else base_types.UninitialisedField(self, 'SsnMgmtReq', SessionManagementRequest9, False)

	@SsnMgmtReq.deleter
	def SsnMgmtReq(self):
		del self._SsnMgmtReq
		self._SsnMgmtReq = base_types.UninitialisedField(self, 'SsnMgmtReq', SessionManagementRequest9, False)

	_field_defs = frozenset((
		base_types.FieldEntry(name='Hdr', type=Header41, min=1, max=1, mutex_group=None, array=False),
		base_types.FieldEntry(name='SctyTrlr', type=ContentInformationType38, min=0, max=1, mutex_group=None, array=False),
		base_types.FieldEntry(name='SsnMgmtReq', type=SessionManagementRequest9, min=1, max=1, mutex_group=None, array=False),
	))