...
class Frequency1(base_types._BaseFieldType):
	...
	def EndPtChc(self, value):
		self._EndPtChc = value if value is not None else base_types.UninitialisedField(self, 'EndPtChc', EndPoint1Choice, False)

	@EndPtChc.deleter
	def EndPtChc(self):
		del self._EndPtChc
		self._EndPtChc = base_types.UninitialisedField(self, 'EndPtChc', EndPoint1Choice, False)
