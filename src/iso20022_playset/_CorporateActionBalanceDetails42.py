...
class CorporateActionBalanceDetails42(base_types._BaseFieldType):
	...
	def PdgDlvryBal(self, value):
		self._PdgDlvryBal = value if value is not None else base_types.UninitialisedField(self, 'PdgDlvryBal', BalanceFormat11Choice, True)

	@PdgDlvryBal.deleter
	def PdgDlvryBal(self):
		del self._PdgDlvryBal
		self._PdgDlvryBal = base_types.UninitialisedField(self, 'PdgDlvryBal', BalanceFormat11Choice, True)
	...
	def PdgRctBal(self, value):
		self._PdgRctBal = value if value is not None else base_types.UninitialisedField(self, 'PdgRctBal', BalanceFormat11Choice, True)

	@PdgRctBal.deleter
	def PdgRctBal(self):
		del self._PdgRctBal
		self._PdgRctBal = base_types.UninitialisedField(self, 'PdgRctBal', BalanceFormat11Choice, True)
	...
	def SttlmPosBal(self, value):
		self._SttlmPosBal = value if value is not None else base_types.UninitialisedField(self, 'SttlmPosBal', BalanceFormat11Choice, False)

	@SttlmPosBal.deleter
	def SttlmPosBal(self):
		del self._SttlmPosBal
		self._SttlmPosBal = base_types.UninitialisedField(self, 'SttlmPosBal', BalanceFormat11Choice, False)
