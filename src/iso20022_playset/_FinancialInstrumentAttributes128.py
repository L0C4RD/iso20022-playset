...
class FinancialInstrumentAttributes128(base_types._BaseFieldType):
	...
	def PctgOfDebtClm(self, value):
		self._PctgOfDebtClm = value if value is not None else base_types.UninitialisedField(self, 'PctgOfDebtClm', RateFormat24Choice, False)

	@PctgOfDebtClm.deleter
	def PctgOfDebtClm(self):
		del self._PctgOfDebtClm
		self._PctgOfDebtClm = base_types.UninitialisedField(self, 'PctgOfDebtClm', RateFormat24Choice, False)
	...
	def WarrtParity(self, value):
		self._WarrtParity = value if value is not None else base_types.UninitialisedField(self, 'WarrtParity', QuantityToQuantityRatio1, False)

	@WarrtParity.deleter
	def WarrtParity(self):
		del self._WarrtParity
		self._WarrtParity = base_types.UninitialisedField(self, 'WarrtParity', QuantityToQuantityRatio1, False)
