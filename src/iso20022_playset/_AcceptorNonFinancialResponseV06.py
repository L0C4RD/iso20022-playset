...
from . import NonFinancialResponseComponent6

class AcceptorNonFinancialResponseV06(base_types._BaseFieldType):
	...
	def NonFinRspn(self, value):
		self._NonFinRspn = value if value is not None else base_types.UninitialisedField(self, 'NonFinRspn', NonFinancialResponseComponent6, False)

	@NonFinRspn.deleter
	def NonFinRspn(self):
		del self._NonFinRspn
		self._NonFinRspn = base_types.UninitialisedField(self, 'NonFinRspn', NonFinancialResponseComponent6, False)
	...
	_field_defs = frozenset((
		...
		base_types.FieldEntry(name='NonFinRspn', type=NonFinancialResponseComponent6, min=1, max=1, mutex_group=None, array=False),
