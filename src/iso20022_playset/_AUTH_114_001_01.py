...
from . import RegulatoryMetadataReportV01
...
class AUTH_114_001_01():
	...
	class Document(base_types._BaseFieldType_Document):
		...
		def RgltryMetadataRpt(self, value):
			self._RgltryMetadataRpt = value if value is not None else base_types.UninitialisedField(self, 'RgltryMetadataRpt', RegulatoryMetadataReportV01, False)

		@RgltryMetadataRpt.deleter
		def RgltryMetadataRpt(self):
			del self._RgltryMetadataRpt
			self._RgltryMetadataRpt = base_types.UninitialisedField(self, 'RgltryMetadataRpt', RegulatoryMetadataReportV01, False)
