...
class TransactionAdviceV07(base_types._BaseFieldType):
	...
	def TxAdvc(self, value):
		self._TxAdvc = value if value is not None else base_types.UninitialisedField(self, 'TxAdvc', AcceptorCompletionAdvice15, False)

	@TxAdvc.deleter
	def TxAdvc(self):
		del self._TxAdvc
		self._TxAdvc = base_types.UninitialisedField(self, 'TxAdvc', AcceptorCompletionAdvice15, False)

	_field_defs = frozenset((
		base_types.FieldEntry(name='Hdr', type=Header70, min=1, max=1, mutex_group=None, array=False),
		base_types.FieldEntry(name='SctyTrlr', type=ContentInformationType37, min=0, max=1, mutex_group=None, array=False),
		base_types.FieldEntry(name='TxAdvc', type=AcceptorCompletionAdvice15, min=1, max=1, mutex_group=None, array=False),
	))