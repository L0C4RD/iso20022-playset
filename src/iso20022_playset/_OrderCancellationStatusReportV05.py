...
from . import Status42Choice

class OrderCancellationStatusReportV05(base_types._BaseFieldType):
	...
	def StsRpt(self, value):
		self._StsRpt = value if value is not None else base_types.UninitialisedField(self, 'StsRpt', Status42Choice, False)

	@StsRpt.deleter
	def StsRpt(self):
		del self._StsRpt
		self._StsRpt = base_types.UninitialisedField(self, 'StsRpt', Status42Choice, False)
	...
	_field_defs = frozenset((
		...
		base_types.FieldEntry(name='StsRpt', type=Status42Choice, min=1, max=1, mutex_group=None, array=False),
