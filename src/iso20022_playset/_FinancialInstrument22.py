...
class FinancialInstrument22(base_types._BaseFieldType):
	...
	def DnmtnCcy(self, value):
		self._DnmtnCcy = value if value is not None else base_types.UninitialisedField(self, 'DnmtnCcy', ActiveCurrencyCode, False)

	@DnmtnCcy.deleter
	def DnmtnCcy(self):
		del self._DnmtnCcy
		self._DnmtnCcy = base_types.UninitialisedField(self, 'DnmtnCcy', ActiveCurrencyCode, False)
	...
	def PdctGrp(self, value):
		self._PdctGrp = value if value is not None else base_types.UninitialisedField(self, 'PdctGrp', RestrictedFINXMax140Text, False)

	@PdctGrp.deleter
	def PdctGrp(self):
		del self._PdctGrp
		self._PdctGrp = base_types.UninitialisedField(self, 'PdctGrp', RestrictedFINXMax140Text, False)
