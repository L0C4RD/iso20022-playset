...
from . import ReservationReturnCriteria1
from . import ReservationSearchCriteria6
...
class ReservationCriteria7(base_types._BaseFieldType):
	...
	def RtrCrit(self, value):
		self._RtrCrit = value if value is not None else base_types.UninitialisedField(self, 'RtrCrit', ReservationReturnCriteria1, False)

	@RtrCrit.deleter
	def RtrCrit(self):
		del self._RtrCrit
		self._RtrCrit = base_types.UninitialisedField(self, 'RtrCrit', ReservationReturnCriteria1, False)
	...
	def SchCrit(self, value):
		self._SchCrit = value if value is not None else base_types.UninitialisedField(self, 'SchCrit', ReservationSearchCriteria6, True)

	@SchCrit.deleter
	def SchCrit(self):
		del self._SchCrit
		self._SchCrit = base_types.UninitialisedField(self, 'SchCrit', ReservationSearchCriteria6, True)
