...
from . import DateCode22Choice
from . import DateCodeAndTimeFormat4
...
class DateFormat54Choice(base_types._BaseFieldType):
	...
	@DtCd.setter
	def DtCd(self, value):
		self._DtCd = value if value is not None else base_types.UninitialisedField(self, 'DtCd', DateCode22Choice, False)

	@DtCd.deleter
	def DtCd(self):
		del self._DtCd
		self._DtCd = base_types.UninitialisedField(self, 'DtCd', DateCode22Choice, False)
	...
	def DtCdAndTm(self, value):
		self._DtCdAndTm = value if value is not None else base_types.UninitialisedField(self, 'DtCdAndTm', DateCodeAndTimeFormat4, False)

	@DtCdAndTm.deleter
	def DtCdAndTm(self):
		del self._DtCdAndTm
		self._DtCdAndTm = base_types.UninitialisedField(self, 'DtCdAndTm', DateCodeAndTimeFormat4, False)
