...
from . import SafekeepingPlaceFormat52Choice

class AccountAndBalance64(base_types._BaseFieldType):
	...
	def SfkpgPlc(self, value):
		self._SfkpgPlc = value if value is not None else base_types.UninitialisedField(self, 'SfkpgPlc', SafekeepingPlaceFormat52Choice, False)

	@SfkpgPlc.deleter
	def SfkpgPlc(self):
		del self._SfkpgPlc
		self._SfkpgPlc = base_types.UninitialisedField(self, 'SfkpgPlc', SafekeepingPlaceFormat52Choice, False)
	...
	_field_defs = frozenset((
		...
		base_types.FieldEntry(name='SfkpgPlc', type=SafekeepingPlaceFormat52Choice, min=0, max=1, mutex_group=None, array=False),
	))