...
class TotalFeesAndTaxes44(base_types._BaseFieldType):

	__slots__ = ["_ComrclAgrmtRef", "_DgtlNtwkFee", "_IndvFee", "_IndvTax", "_TtlFees", "_TtlOvrhdApld", "_TtlTaxs"]
	@property
	...
	def IndvFee(self, value):
		self._IndvFee = value if value is not None else base_types.UninitialisedField(self, 'IndvFee', Fee11, True)

	@IndvFee.deleter
	def IndvFee(self):
		del self._IndvFee
		self._IndvFee = base_types.UninitialisedField(self, 'IndvFee', Fee11, True)
	...
	def IndvTax(self, value):
		self._IndvTax = value if value is not None else base_types.UninitialisedField(self, 'IndvTax', Tax35, True)

	@IndvTax.deleter
	def IndvTax(self):
		del self._IndvTax
		self._IndvTax = base_types.UninitialisedField(self, 'IndvTax', Tax35, True)
	...
	_field_defs = frozenset((
		...
		base_types.FieldEntry(name='DgtlNtwkFee', type=DigitalPaymentSettlement3, min=0, max=2, mutex_group=None, array=True),
		base_types.FieldEntry(name='IndvFee', type=Fee11, min=0, max=None, mutex_group=None, array=True),
