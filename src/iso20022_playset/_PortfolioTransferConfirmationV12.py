...
from . import PortfolioTransfer15

class PortfolioTransferConfirmationV12(base_types._BaseFieldType):
	...
	def PdctTrf(self, value):
		self._PdctTrf = value if value is not None else base_types.UninitialisedField(self, 'PdctTrf', PortfolioTransfer15, True)

	@PdctTrf.deleter
	def PdctTrf(self):
		del self._PdctTrf
		self._PdctTrf = base_types.UninitialisedField(self, 'PdctTrf', PortfolioTransfer15, True)
	...
	_field_defs = frozenset((
		...
		base_types.FieldEntry(name='PdctTrf', type=PortfolioTransfer15, min=1, max=None, mutex_group=None, array=True),
