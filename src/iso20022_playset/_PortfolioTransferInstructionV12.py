...
from . import PortfolioTransfer14

class PortfolioTransferInstructionV12(base_types._BaseFieldType):
	...
	def PdctTrf(self, value):
		self._PdctTrf = value if value is not None else base_types.UninitialisedField(self, 'PdctTrf', PortfolioTransfer14, True)

	@PdctTrf.deleter
	def PdctTrf(self):
		del self._PdctTrf
		self._PdctTrf = base_types.UninitialisedField(self, 'PdctTrf', PortfolioTransfer14, True)
	...
	_field_defs = frozenset((
		...
		base_types.FieldEntry(name='PdctTrf', type=PortfolioTransfer14, min=1, max=None, mutex_group=None, array=True),
