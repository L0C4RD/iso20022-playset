...
class RentalRate1(base_types._BaseFieldType):
	...
	@Prd.setter
	def Prd(self, value):
		self._Prd = value if value is not None else base_types.UninitialisedField(self, 'Prd', PeriodUnit3Code, False)

	@Prd.deleter
	def Prd(self):
		del self._Prd
		self._Prd = base_types.UninitialisedField(self, 'Prd', PeriodUnit3Code, False)
