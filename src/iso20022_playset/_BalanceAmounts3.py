...
class BalanceAmounts3(base_types._BaseFieldType):
	...
	def ElgblCollVal(self, value):
		self._ElgblCollVal = value if value is not None else base_types.UninitialisedField(self, 'ElgblCollVal', AmountAndDirection6, False)

	@ElgblCollVal.deleter
	def ElgblCollVal(self):
		del self._ElgblCollVal
		self._ElgblCollVal = base_types.UninitialisedField(self, 'ElgblCollVal', AmountAndDirection6, False)
