...
from . import MarginReportData10
...
class TradeData62Choice(base_types._BaseFieldType):
	...
	@Stat.setter
	def Stat(self, value):
		self._Stat = value if value is not None else base_types.UninitialisedField(self, 'Stat', MarginReportData10, True)

	@Stat.deleter
	def Stat(self):
		del self._Stat
		self._Stat = base_types.UninitialisedField(self, 'Stat', MarginReportData10, True)

	_field_defs = frozenset((
		base_types.FieldEntry(name='DataSetActn', type=ReportPeriodActivity1Code, min=0, max=1, mutex_group=1, array=False),
		base_types.FieldEntry(name='Stat', type=MarginReportData10, min=1, max=None, mutex_group=1, array=True),
	))