...
class MeetingInstructionV11(base_types._BaseFieldType):
	...
	def Instr(self, value):
		self._Instr = value if value is not None else base_types.UninitialisedField(self, 'Instr', Instruction9, True)

	@Instr.deleter
	def Instr(self):
		del self._Instr
		self._Instr = base_types.UninitialisedField(self, 'Instr', Instruction9, True)
	...
	def MtgRef(self, value):
		self._MtgRef = value if value is not None else base_types.UninitialisedField(self, 'MtgRef', MeetingReference11, False)

	@MtgRef.deleter
	def MtgRef(self):
		del self._MtgRef
		self._MtgRef = base_types.UninitialisedField(self, 'MtgRef', MeetingReference11, False)
	...
	_field_defs = frozenset((
		...
		base_types.FieldEntry(name='Instr', type=Instruction9, min=1, max=None, mutex_group=None, array=True),
		...
		base_types.FieldEntry(name='MtgRef', type=MeetingReference11, min=1, max=1, mutex_group=None, array=False),
