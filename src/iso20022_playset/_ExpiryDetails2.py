...
class ExpiryDetails2(base_types._BaseFieldType):
	...
	def XpryTerms(self, value):
		self._XpryTerms = value if value is not None else base_types.UninitialisedField(self, 'XpryTerms', ExpiryTerms2, False)

	@XpryTerms.deleter
	def XpryTerms(self):
		del self._XpryTerms
		self._XpryTerms = base_types.UninitialisedField(self, 'XpryTerms', ExpiryTerms2, False)
