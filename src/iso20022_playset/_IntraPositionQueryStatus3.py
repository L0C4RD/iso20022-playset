...
from . import IntraPositionStatusType2

class IntraPositionQueryStatus3(base_types._BaseFieldType):
	...
	@Tp.setter
	def Tp(self, value):
		self._Tp = value if value is not None else base_types.UninitialisedField(self, 'Tp', IntraPositionStatusType2, False)

	@Tp.deleter
	def Tp(self):
		del self._Tp
		self._Tp = base_types.UninitialisedField(self, 'Tp', IntraPositionStatusType2, False)

	_field_defs = frozenset((
		base_types.FieldEntry(name='DtPrd', type=DateAndDateTimeSearch5Choice, min=0, max=1, mutex_group=None, array=False),
		base_types.FieldEntry(name='Tp', type=IntraPositionStatusType2, min=1, max=1, mutex_group=None, array=False),
	))