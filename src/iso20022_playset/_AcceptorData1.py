...
class AcceptorData1(base_types._BaseFieldType):
	...
	def LclData(self, value):
		self._LclData = value if value is not None else base_types.UninitialisedField(self, 'LclData', LocalData19, True)

	@LclData.deleter
	def LclData(self):
		del self._LclData
		self._LclData = base_types.UninitialisedField(self, 'LclData', LocalData19, True)
	...
	def SvcLctn(self, value):
		self._SvcLctn = value if value is not None else base_types.UninitialisedField(self, 'SvcLctn', Address4, False)

	@SvcLctn.deleter
	def SvcLctn(self):
		del self._SvcLctn
		self._SvcLctn = base_types.UninitialisedField(self, 'SvcLctn', Address4, False)
