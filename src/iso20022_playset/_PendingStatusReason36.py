...
from . import PendingReason83Choice

class PendingStatusReason36(base_types._BaseFieldType):
	...
	def RsnCd(self, value):
		self._RsnCd = value if value is not None else base_types.UninitialisedField(self, 'RsnCd', PendingReason83Choice, False)

	@RsnCd.deleter
	def RsnCd(self):
		del self._RsnCd
		self._RsnCd = base_types.UninitialisedField(self, 'RsnCd', PendingReason83Choice, False)

	_field_defs = frozenset((
		base_types.FieldEntry(name='AddtlRsnInf', type=Max210Text, min=0, max=1, mutex_group=None, array=False),
		base_types.FieldEntry(name='RsnCd', type=PendingReason83Choice, min=1, max=1, mutex_group=None, array=False),
	))