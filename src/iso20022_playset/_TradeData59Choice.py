...
from . import TradeReport33Choice

class TradeData59Choice(base_types._BaseFieldType):
	...
	@Rpt.setter
	def Rpt(self, value):
		self._Rpt = value if value is not None else base_types.UninitialisedField(self, 'Rpt', TradeReport33Choice, True)

	@Rpt.deleter
	def Rpt(self):
		del self._Rpt
		self._Rpt = base_types.UninitialisedField(self, 'Rpt', TradeReport33Choice, True)

	_field_defs = frozenset((
		base_types.FieldEntry(name='DataSetActn', type=ReportPeriodActivity1Code, min=0, max=1, mutex_group=1, array=False),
		base_types.FieldEntry(name='Rpt', type=TradeReport33Choice, min=1, max=None, mutex_group=1, array=True),
	))