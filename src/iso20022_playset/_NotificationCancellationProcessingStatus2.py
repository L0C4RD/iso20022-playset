...
from . import ProcessedStatus2Format1Choice
...
class NotificationCancellationProcessingStatus2(base_types._BaseFieldType):
	...
	@Sts.setter
	def Sts(self, value):
		self._Sts = value if value is not None else base_types.UninitialisedField(self, 'Sts', ProcessedStatus2Format1Choice, False)

	@Sts.deleter
	def Sts(self):
		del self._Sts
		self._Sts = base_types.UninitialisedField(self, 'Sts', ProcessedStatus2Format1Choice, False)

	_field_defs = frozenset((
		base_types.FieldEntry(name='AddtlInf', type=Max350Text, min=0, max=1, mutex_group=None, array=False),
		base_types.FieldEntry(name='Sts', type=ProcessedStatus2Format1Choice, min=1, max=1, mutex_group=None, array=False),
	))