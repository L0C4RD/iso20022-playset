...
from . import TaxAmount2
from . import TaxPeriod2
...
class TaxRecord2(base_types._BaseFieldType):
	...
	@Prd.setter
	def Prd(self, value):
		self._Prd = value if value is not None else base_types.UninitialisedField(self, 'Prd', TaxPeriod2, False)

	@Prd.deleter
	def Prd(self):
		del self._Prd
		self._Prd = base_types.UninitialisedField(self, 'Prd', TaxPeriod2, False)
	...
	def TaxAmt(self, value):
		self._TaxAmt = value if value is not None else base_types.UninitialisedField(self, 'TaxAmt', TaxAmount2, False)

	@TaxAmt.deleter
	def TaxAmt(self):
		del self._TaxAmt
		self._TaxAmt = base_types.UninitialisedField(self, 'TaxAmt', TaxAmount2, False)
