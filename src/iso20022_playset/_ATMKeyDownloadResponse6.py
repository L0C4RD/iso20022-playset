...
from . import SecurityParameters19

class ATMKeyDownloadResponse6(base_types._BaseFieldType):
	...
	def HstSctyParams(self, value):
		self._HstSctyParams = value if value is not None else base_types.UninitialisedField(self, 'HstSctyParams', SecurityParameters19, False)

	@HstSctyParams.deleter
	def HstSctyParams(self):
		del self._HstSctyParams
		self._HstSctyParams = base_types.UninitialisedField(self, 'HstSctyParams', SecurityParameters19, False)
	...
	_field_defs = frozenset((
		...
		base_types.FieldEntry(name='HstSctyParams', type=SecurityParameters19, min=1, max=1, mutex_group=None, array=False),
	))