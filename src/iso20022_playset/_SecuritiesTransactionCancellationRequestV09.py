...
from . import CancellationReason23
from . import FXCancellation3Choice
from . import Max35Text
from . import PartyIdentification144
from . import References45Choice
from . import SecuritiesAccount19
from . import SupplementaryData1
from . import TransactionDetails180

class SecuritiesTransactionCancellationRequestV09(base_types._BaseFieldType):

	__slots__ = ["_AcctOwnr", "_AcctOwnrTxId", "_AcctSvcrTxId", "_BlckChainAdrOrWllt", "_CxlRsn", "_FxCxl", "_MktInfrstrctrTxId", "_NonceId", "_PrcrTxId", "_SfkpgAcct", "_SplmtryData", "_TxDtls"]
	@property
	def AcctOwnr(self):
	...
	def AcctOwnrTxId(self, value):
		self._AcctOwnrTxId = value if value is not None else base_types.UninitialisedField(self, 'AcctOwnrTxId', References45Choice, False)

	@AcctOwnrTxId.deleter
	def AcctOwnrTxId(self):
		del self._AcctOwnrTxId
		self._AcctOwnrTxId = base_types.UninitialisedField(self, 'AcctOwnrTxId', References45Choice, False)
	...
	def CxlRsn(self, value):
		self._CxlRsn = value if value is not None else base_types.UninitialisedField(self, 'CxlRsn', CancellationReason23, False)

	@CxlRsn.deleter
	def CxlRsn(self):
		del self._CxlRsn
		self._CxlRsn = base_types.UninitialisedField(self, 'CxlRsn', CancellationReason23, False)
	...
	def FxCxl(self, value):
		self._FxCxl = value if value is not None else base_types.UninitialisedField(self, 'FxCxl', FXCancellation3Choice, False)

	@FxCxl.deleter
	def FxCxl(self):
		del self._FxCxl
		self._FxCxl = base_types.UninitialisedField(self, 'FxCxl', FXCancellation3Choice, False)
	...
	def TxDtls(self, value):
		self._TxDtls = value if value is not None else base_types.UninitialisedField(self, 'TxDtls', TransactionDetails180, False)

	@TxDtls.deleter
	def TxDtls(self):
		del self._TxDtls
		self._TxDtls = base_types.UninitialisedField(self, 'TxDtls', TransactionDetails180, False)
	...
	_field_defs = frozenset((
		...
		base_types.FieldEntry(name='TxDtls', type=TransactionDetails180, min=0, max=1, mutex_group=None, array=False),
	))