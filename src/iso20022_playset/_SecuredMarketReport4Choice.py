...
from . import SecuredMarketTransaction4
...
class SecuredMarketReport4Choice(base_types._BaseFieldType):
	...
	@Tx.setter
	def Tx(self, value):
		self._Tx = value if value is not None else base_types.UninitialisedField(self, 'Tx', SecuredMarketTransaction4, True)

	@Tx.deleter
	def Tx(self):
		del self._Tx
		self._Tx = base_types.UninitialisedField(self, 'Tx', SecuredMarketTransaction4, True)

	_field_defs = frozenset((
		base_types.FieldEntry(name='DataSetActn', type=ReportPeriodActivity3Code, min=0, max=1, mutex_group=1, array=False),
		base_types.FieldEntry(name='Tx', type=SecuredMarketTransaction4, min=1, max=None, mutex_group=1, array=True),
	))