...
from . import LimitReturnCriteria2
from . import LimitSearchCriteria7
...
class LimitCriteria7(base_types._BaseFieldType):
	...
	def RtrCrit(self, value):
		self._RtrCrit = value if value is not None else base_types.UninitialisedField(self, 'RtrCrit', LimitReturnCriteria2, False)

	@RtrCrit.deleter
	def RtrCrit(self):
		del self._RtrCrit
		self._RtrCrit = base_types.UninitialisedField(self, 'RtrCrit', LimitReturnCriteria2, False)
	...
	def SchCrit(self, value):
		self._SchCrit = value if value is not None else base_types.UninitialisedField(self, 'SchCrit', LimitSearchCriteria7, True)

	@SchCrit.deleter
	def SchCrit(self):
		del self._SchCrit
		self._SchCrit = base_types.UninitialisedField(self, 'SchCrit', LimitSearchCriteria7, True)
