...
class SecuritiesTransactionStatusQueryV08(base_types._BaseFieldType):
	...
	def StsAdvcReqd(self, value):
		self._StsAdvcReqd = value if value is not None else base_types.UninitialisedField(self, 'StsAdvcReqd', DocumentNumber22, False)

	@StsAdvcReqd.deleter
	def StsAdvcReqd(self):
		del self._StsAdvcReqd
		self._StsAdvcReqd = base_types.UninitialisedField(self, 'StsAdvcReqd', DocumentNumber22, False)
	...
	_field_defs = frozenset((
		...
		base_types.FieldEntry(name='StsAdvcReqd', type=DocumentNumber22, min=1, max=1, mutex_group=None, array=False),
	))