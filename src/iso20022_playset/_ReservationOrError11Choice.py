...
from . import CurrentAndDefaultReservation6
...
class ReservationOrError11Choice(base_types._BaseFieldType):
	...
	def BizRpt(self, value):
		self._BizRpt = value if value is not None else base_types.UninitialisedField(self, 'BizRpt', CurrentAndDefaultReservation6, False)

	@BizRpt.deleter
	def BizRpt(self):
		del self._BizRpt
		self._BizRpt = base_types.UninitialisedField(self, 'BizRpt', CurrentAndDefaultReservation6, False)
	...
	_field_defs = frozenset((
		base_types.FieldEntry(name='BizRpt', type=CurrentAndDefaultReservation6, min=0, max=1, mutex_group=1, array=False),
