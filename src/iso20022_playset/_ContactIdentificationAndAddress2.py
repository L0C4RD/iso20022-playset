...
from . import PaymentRole1Choice
...
class ContactIdentificationAndAddress2(base_types._BaseFieldType):
	...
	@Role.setter
	def Role(self, value):
		self._Role = value if value is not None else base_types.UninitialisedField(self, 'Role', PaymentRole1Choice, False)

	@Role.deleter
	def Role(self):
		del self._Role
		self._Role = base_types.UninitialisedField(self, 'Role', PaymentRole1Choice, False)
