...
from . import PaymentTransaction176

class OriginalPaymentInstruction55(base_types._BaseFieldType):
	...
	def TxInf(self, value):
		self._TxInf = value if value is not None else base_types.UninitialisedField(self, 'TxInf', PaymentTransaction176, True)

	@TxInf.deleter
	def TxInf(self):
		del self._TxInf
		self._TxInf = base_types.UninitialisedField(self, 'TxInf', PaymentTransaction176, True)
	...
	_field_defs = frozenset((
		...
		base_types.FieldEntry(name='TxInf', type=PaymentTransaction176, min=0, max=None, mutex_group=None, array=True),
	))