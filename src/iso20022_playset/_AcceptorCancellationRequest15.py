...
from . import CardPaymentTransaction150
from . import PaymentContext30

class AcceptorCancellationRequest15(base_types._BaseFieldType):
	...
	@Tx.setter
	def Tx(self, value):
		self._Tx = value if value is not None else base_types.UninitialisedField(self, 'Tx', CardPaymentTransaction150, False)

	@Tx.deleter
	def Tx(self):
		del self._Tx
		self._Tx = base_types.UninitialisedField(self, 'Tx', CardPaymentTransaction150, False)
	...
	_field_defs = frozenset((
		...
		base_types.FieldEntry(name='Tx', type=CardPaymentTransaction150, min=1, max=1, mutex_group=None, array=False),
	))