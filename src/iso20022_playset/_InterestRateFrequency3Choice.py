...
class InterestRateFrequency3Choice(base_types._BaseFieldType):
	...
	@Term.setter
	def Term(self, value):
		self._Term = value if value is not None else base_types.UninitialisedField(self, 'Term', InterestRateContractTerm4, False)

	@Term.deleter
	def Term(self):
		del self._Term
		self._Term = base_types.UninitialisedField(self, 'Term', InterestRateContractTerm4, False)
