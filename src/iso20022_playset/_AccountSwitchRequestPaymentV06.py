...
class AccountSwitchRequestPaymentV06(base_types._BaseFieldType):
	...
	def CdtInstr(self, value):
		self._CdtInstr = value if value is not None else base_types.UninitialisedField(self, 'CdtInstr', CreditTransferTransaction78, False)

	@CdtInstr.deleter
	def CdtInstr(self):
		del self._CdtInstr
		self._CdtInstr = base_types.UninitialisedField(self, 'CdtInstr', CreditTransferTransaction78, False)
	...
	_field_defs = frozenset((
		...
		base_types.FieldEntry(name='CdtInstr', type=CreditTransferTransaction78, min=1, max=1, mutex_group=None, array=False),
