...
class DuplicateV07(base_types._BaseFieldType):
	...
	def Dplct(self, value):
		self._Dplct = value if value is not None else base_types.UninitialisedField(self, 'Dplct', ProprietaryData7, False)

	@Dplct.deleter
	def Dplct(self):
		del self._Dplct
		self._Dplct = base_types.UninitialisedField(self, 'Dplct', ProprietaryData7, False)
