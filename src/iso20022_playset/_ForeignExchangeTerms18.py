...
class ForeignExchangeTerms18(base_types._BaseFieldType):
	...
	def ConvtdAmt(self, value):
		self._ConvtdAmt = value if value is not None else base_types.UninitialisedField(self, 'ConvtdAmt', ActiveCurrencyAndAmount, False)

	@ConvtdAmt.deleter
	def ConvtdAmt(self):
		del self._ConvtdAmt
		self._ConvtdAmt = base_types.UninitialisedField(self, 'ConvtdAmt', ActiveCurrencyAndAmount, False)
