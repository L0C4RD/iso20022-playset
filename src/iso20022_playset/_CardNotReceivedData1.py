...
from . import CardSecurityCapability2Code
from . import ISODate
...
class CardNotReceivedData1(base_types._BaseFieldType):
	...
	def CardSctyCpblty(self, value):
		self._CardSctyCpblty = value if value is not None else base_types.UninitialisedField(self, 'CardSctyCpblty', CardSecurityCapability2Code, True)

	@CardSctyCpblty.deleter
	def CardSctyCpblty(self):
		del self._CardSctyCpblty
		self._CardSctyCpblty = base_types.UninitialisedField(self, 'CardSctyCpblty', CardSecurityCapability2Code, True)
	...
	def MlngAdr(self, value):
		self._MlngAdr = value if value is not None else base_types.UninitialisedField(self, 'MlngAdr', Address4, False)

	@MlngAdr.deleter
	def MlngAdr(self):
		del self._MlngAdr
		self._MlngAdr = base_types.UninitialisedField(self, 'MlngAdr', Address4, False)
