...
from . import OrderStatus8Choice
from . import PartyIdentification139

class OrderStatusAndReason11(base_types._BaseFieldType):
	...
	def OrdrSts(self, value):
		self._OrdrSts = value if value is not None else base_types.UninitialisedField(self, 'OrdrSts', OrderStatus8Choice, False)

	@OrdrSts.deleter
	def OrdrSts(self):
		del self._OrdrSts
		self._OrdrSts = base_types.UninitialisedField(self, 'OrdrSts', OrderStatus8Choice, False)
	...
	_field_defs = frozenset((
		...
		base_types.FieldEntry(name='OrdrSts', type=OrderStatus8Choice, min=1, max=1, mutex_group=None, array=False),
