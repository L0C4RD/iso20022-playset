...
from . import PartyIdentification45
from . import TaxParty3
...
class TradeParty1(base_types._BaseFieldType):
	...
	def PtyId(self, value):
		self._PtyId = value if value is not None else base_types.UninitialisedField(self, 'PtyId', PartyIdentification45, False)

	@PtyId.deleter
	def PtyId(self):
		del self._PtyId
		self._PtyId = base_types.UninitialisedField(self, 'PtyId', PartyIdentification45, False)
	...
	def TaxPty(self, value):
		self._TaxPty = value if value is not None else base_types.UninitialisedField(self, 'TaxPty', TaxParty3, True)

	@TaxPty.deleter
	def TaxPty(self):
		del self._TaxPty
		self._TaxPty = base_types.UninitialisedField(self, 'TaxPty', TaxParty3, True)
