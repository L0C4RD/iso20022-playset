...
from . import BusinessFlowDirectionType1Code
...
from . import PortfolioTransfer17

class AccountHoldingInformationRequestV09(base_types._BaseFieldType):
	...
	def BizFlowDrctnTp(self, value):
		self._BizFlowDrctnTp = value if value is not None else base_types.UninitialisedField(self, 'BizFlowDrctnTp', BusinessFlowDirectionType1Code, False)

	@BizFlowDrctnTp.deleter
	def BizFlowDrctnTp(self):
		del self._BizFlowDrctnTp
		self._BizFlowDrctnTp = base_types.UninitialisedField(self, 'BizFlowDrctnTp', BusinessFlowDirectionType1Code, False)
	...
	def PdctTrf(self, value):
		self._PdctTrf = value if value is not None else base_types.UninitialisedField(self, 'PdctTrf', PortfolioTransfer17, True)

	@PdctTrf.deleter
	def PdctTrf(self):
		del self._PdctTrf
		self._PdctTrf = base_types.UninitialisedField(self, 'PdctTrf', PortfolioTransfer17, True)
	...
	_field_defs = frozenset((
		...
		base_types.FieldEntry(name='PdctTrf', type=PortfolioTransfer17, min=1, max=None, mutex_group=None, array=True),
