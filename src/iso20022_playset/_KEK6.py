...
from . import AlgorithmIdentification28
...
class KEK6(base_types._BaseFieldType):
	...
	def KEKId(self, value):
		self._KEKId = value if value is not None else base_types.UninitialisedField(self, 'KEKId', KEKIdentifier6, False)

	@KEKId.deleter
	def KEKId(self):
		del self._KEKId
		self._KEKId = base_types.UninitialisedField(self, 'KEKId', KEKIdentifier6, False)
	...
	def KeyNcrptnAlgo(self, value):
		self._KeyNcrptnAlgo = value if value is not None else base_types.UninitialisedField(self, 'KeyNcrptnAlgo', AlgorithmIdentification28, False)

	@KeyNcrptnAlgo.deleter
	def KeyNcrptnAlgo(self):
		del self._KeyNcrptnAlgo
		self._KeyNcrptnAlgo = base_types.UninitialisedField(self, 'KeyNcrptnAlgo', AlgorithmIdentification28, False)
