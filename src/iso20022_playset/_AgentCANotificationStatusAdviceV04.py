...
from . import AgentDocumentIdentificationAndStatus2Choice
from . import CorporateActionGeneralInformation175

class AgentCANotificationStatusAdviceV04(base_types._BaseFieldType):
	...
	def AgtDocIdAndSts(self, value):
		self._AgtDocIdAndSts = value if value is not None else base_types.UninitialisedField(self, 'AgtDocIdAndSts', AgentDocumentIdentificationAndStatus2Choice, False)

	@AgtDocIdAndSts.deleter
	def AgtDocIdAndSts(self):
		del self._AgtDocIdAndSts
		self._AgtDocIdAndSts = base_types.UninitialisedField(self, 'AgtDocIdAndSts', AgentDocumentIdentificationAndStatus2Choice, False)
