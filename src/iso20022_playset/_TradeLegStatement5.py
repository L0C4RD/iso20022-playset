...
from . import TradeLeg15

class TradeLegStatement5(base_types._BaseFieldType):
	...
	def TradLegsDtls(self, value):
		self._TradLegsDtls = value if value is not None else base_types.UninitialisedField(self, 'TradLegsDtls', TradeLeg15, True)

	@TradLegsDtls.deleter
	def TradLegsDtls(self):
		del self._TradLegsDtls
		self._TradLegsDtls = base_types.UninitialisedField(self, 'TradLegsDtls', TradeLeg15, True)
	...
	_field_defs = frozenset((
		...
		base_types.FieldEntry(name='TradLegsDtls', type=TradeLeg15, min=1, max=None, mutex_group=None, array=True),
	))