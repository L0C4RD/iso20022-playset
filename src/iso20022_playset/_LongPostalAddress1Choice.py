...
from . import StructuredLongPostalAddress1
...
class LongPostalAddress1Choice(base_types._BaseFieldType):
	...
	@Strd.setter
	def Strd(self, value):
		self._Strd = value if value is not None else base_types.UninitialisedField(self, 'Strd', StructuredLongPostalAddress1, False)

	@Strd.deleter
	def Strd(self):
		del self._Strd
		self._Strd = base_types.UninitialisedField(self, 'Strd', StructuredLongPostalAddress1, False)
	...
	_field_defs = frozenset((
		base_types.FieldEntry(name='Strd', type=StructuredLongPostalAddress1, min=0, max=1, mutex_group=1, array=False),
