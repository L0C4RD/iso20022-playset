...
from . import InvestmentFundTransaction4
from . import Max350Text
from . import Max35Text
from . import PaginationBalance2
from . import SecurityIdentification3Choice

class InvestmentFundTransactionsByFund3(base_types._BaseFieldType):

	__slots__ = ["_BalByPg", "_ClssTp", "_DstrbtnPlcy", "_Id", "_Nm", "_SctiesForm", "_SplmtryId", "_TxDtls"]
	@property
	def BalByPg(self):
	...
	def BalByPg(self, value):
		self._BalByPg = value if value is not None else base_types.UninitialisedField(self, 'BalByPg', PaginationBalance2, False)

	@BalByPg.deleter
	def BalByPg(self):
		del self._BalByPg
		self._BalByPg = base_types.UninitialisedField(self, 'BalByPg', PaginationBalance2, False)
	...
	@Id.setter
	def Id(self, value):
		self._Id = value if value is not None else base_types.UninitialisedField(self, 'Id', SecurityIdentification3Choice, False)

	@Id.deleter
	def Id(self):
		del self._Id
		self._Id = base_types.UninitialisedField(self, 'Id', SecurityIdentification3Choice, False)
	...
	def TxDtls(self, value):
		self._TxDtls = value if value is not None else base_types.UninitialisedField(self, 'TxDtls', InvestmentFundTransaction4, True)

	@TxDtls.deleter
	def TxDtls(self):
		del self._TxDtls
		self._TxDtls = base_types.UninitialisedField(self, 'TxDtls', InvestmentFundTransaction4, True)
