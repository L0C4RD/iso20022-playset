...
class ExpectedCollateral2(base_types._BaseFieldType):
	...
	def VartnMrgn(self, value):
		self._VartnMrgn = value if value is not None else base_types.UninitialisedField(self, 'VartnMrgn', ExpectedCollateralMovement2, False)

	@VartnMrgn.deleter
	def VartnMrgn(self):
		del self._VartnMrgn
		self._VartnMrgn = base_types.UninitialisedField(self, 'VartnMrgn', ExpectedCollateralMovement2, False)
