...
from . import Transaction165

class FinancialInstrumentDetails48(base_types._BaseFieldType):
	...
	@Tx.setter
	def Tx(self, value):
		self._Tx = value if value is not None else base_types.UninitialisedField(self, 'Tx', Transaction165, True)

	@Tx.deleter
	def Tx(self):
		del self._Tx
		self._Tx = base_types.UninitialisedField(self, 'Tx', Transaction165, True)
	...
	_field_defs = frozenset((
		...
		base_types.FieldEntry(name='Tx', type=Transaction165, min=1, max=None, mutex_group=None, array=True),
	))