...
from . import UniqueTransactionIdentifier1Choice

class ClearingPartyAndTime23(base_types._BaseFieldType):

	__slots__ = ["_CCP", "_ClrDtTm", "_ClrIdr", "_ClrRctDtTm", "_OrgnlIdr", "_OrgnlTradRpstryIdr"]
	@property
	def CCP(self):
		return self._CCP
	...
	def ClrIdr(self, value):
		self._ClrIdr = value if value is not None else base_types.UninitialisedField(self, 'ClrIdr', UniqueTransactionIdentifier1Choice, False)

	@ClrIdr.deleter
	def ClrIdr(self):
		del self._ClrIdr
		self._ClrIdr = base_types.UninitialisedField(self, 'ClrIdr', UniqueTransactionIdentifier1Choice, False)
	...
	def OrgnlIdr(self, value):
		self._OrgnlIdr = value if value is not None else base_types.UninitialisedField(self, 'OrgnlIdr', UniqueTransactionIdentifier1Choice, False)

	@OrgnlIdr.deleter
	def OrgnlIdr(self):
		del self._OrgnlIdr
		self._OrgnlIdr = base_types.UninitialisedField(self, 'OrgnlIdr', UniqueTransactionIdentifier1Choice, False)
