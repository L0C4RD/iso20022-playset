...
class ForeignExchangeTerms6(base_types._BaseFieldType):
	...
	def QtgInstn(self, value):
		self._QtgInstn = value if value is not None else base_types.UninitialisedField(self, 'QtgInstn', PartyIdentification2Choice, False)

	@QtgInstn.deleter
	def QtgInstn(self):
		del self._QtgInstn
		self._QtgInstn = base_types.UninitialisedField(self, 'QtgInstn', PartyIdentification2Choice, False)
