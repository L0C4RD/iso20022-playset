...
from . import TransactionLoanData26Choice

class TradeTransactionCollateralUpdate8(base_types._BaseFieldType):

	__slots__ = ["_CollData", "_CtrPtySpcfcData", "_LnData", "_SplmtryData", "_TechRcrdId"]
	@property
	def CollData(self):
	...
	def LnData(self, value):
		self._LnData = value if value is not None else base_types.UninitialisedField(self, 'LnData', TransactionLoanData26Choice, False)

	@LnData.deleter
	def LnData(self):
		del self._LnData
		self._LnData = base_types.UninitialisedField(self, 'LnData', TransactionLoanData26Choice, False)
