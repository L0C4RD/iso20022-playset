...
from . import OtherIdentification8

class SecurityIdentification50(base_types._BaseFieldType):
	...
	def OthrId(self, value):
		self._OthrId = value if value is not None else base_types.UninitialisedField(self, 'OthrId', OtherIdentification8, True)

	@OthrId.deleter
	def OthrId(self):
		del self._OthrId
		self._OthrId = base_types.UninitialisedField(self, 'OthrId', OtherIdentification8, True)
	...
	_field_defs = frozenset((
		...
		base_types.FieldEntry(name='OthrId', type=OtherIdentification8, min=0, max=None, mutex_group=None, array=True),
	))