...
from . import PartyIdentification262Choice
...
class PartyIdentification339(base_types._BaseFieldType):
	...
	@Pty.setter
	def Pty(self, value):
		self._Pty = value if value is not None else base_types.UninitialisedField(self, 'Pty', PartyIdentification262Choice, False)

	@Pty.deleter
	def Pty(self):
		del self._Pty
		self._Pty = base_types.UninitialisedField(self, 'Pty', PartyIdentification262Choice, False)
