...
from . import TaxEfficientProduct6
...
class FundPortfolio8Choice(base_types._BaseFieldType):
	...
	def TaxEffcntPdct(self, value):
		self._TaxEffcntPdct = value if value is not None else base_types.UninitialisedField(self, 'TaxEffcntPdct', TaxEfficientProduct6, False)

	@TaxEffcntPdct.deleter
	def TaxEffcntPdct(self):
		del self._TaxEffcntPdct
		self._TaxEffcntPdct = base_types.UninitialisedField(self, 'TaxEffcntPdct', TaxEfficientProduct6, False)
	...
	_field_defs = frozenset((
		...
		base_types.FieldEntry(name='TaxEffcntPdct', type=TaxEfficientProduct6, min=0, max=1, mutex_group=1, array=False),
	))