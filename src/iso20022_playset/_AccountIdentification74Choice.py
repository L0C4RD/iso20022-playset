...
from . import AccountIdentification72
...
class AccountIdentification74Choice(base_types._BaseFieldType):
	...
	def AcctsList(self, value):
		self._AcctsList = value if value is not None else base_types.UninitialisedField(self, 'AcctsList', AccountIdentification72, True)

	@AcctsList.deleter
	def AcctsList(self):
		del self._AcctsList
		self._AcctsList = base_types.UninitialisedField(self, 'AcctsList', AccountIdentification72, True)
	...
	_field_defs = frozenset((
		base_types.FieldEntry(name='AcctsList', type=AccountIdentification72, min=1, max=None, mutex_group=1, array=True),
