...
from . import StatementStructure1Code
from . import UpdateType16Choice
from . import YesNoIndicator

class Statement70(base_types._BaseFieldType):

	__slots__ = ["_ActvtyInd", "_Frqcy", "_QryRef", "_RptNb", "_StmtDtTm", "_StmtId", "_StmtStr", "_UpdTp"]
	@property
	def ActvtyInd(self):
	...
	def StmtStr(self, value):
		self._StmtStr = value if value is not None else base_types.UninitialisedField(self, 'StmtStr', StatementStructure1Code, False)

	@StmtStr.deleter
	def StmtStr(self):
		del self._StmtStr
		self._StmtStr = base_types.UninitialisedField(self, 'StmtStr', StatementStructure1Code, False)
