...
from . import CreditorEnrolmentStatusReason2Choice
...
class CreditorEnrolmentStatusReason3(base_types._BaseFieldType):
	...
	@Rsn.setter
	def Rsn(self, value):
		self._Rsn = value if value is not None else base_types.UninitialisedField(self, 'Rsn', CreditorEnrolmentStatusReason2Choice, False)

	@Rsn.deleter
	def Rsn(self):
		del self._Rsn
		self._Rsn = base_types.UninitialisedField(self, 'Rsn', CreditorEnrolmentStatusReason2Choice, False)
