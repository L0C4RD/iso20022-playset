...
class SystemAbort8(base_types._BaseFieldType):
	...
	def SysAbrt(self, value):
		self._SysAbrt = value if value is not None else base_types.UninitialisedField(self, 'SysAbrt', AbortData6, False)

	@SysAbrt.deleter
	def SysAbrt(self):
		del self._SysAbrt
		self._SysAbrt = base_types.UninitialisedField(self, 'SysAbrt', AbortData6, False)
