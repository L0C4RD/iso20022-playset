...
class TransactionIdentifications37(base_types._BaseFieldType):
	...
	def OthrId(self, value):
		self._OthrId = value if value is not None else base_types.UninitialisedField(self, 'OthrId', RestrictedFINXMax16Text, False)

	@OthrId.deleter
	def OthrId(self):
		del self._OthrId
		self._OthrId = base_types.UninitialisedField(self, 'OthrId', RestrictedFINXMax16Text, False)
	...
	_field_defs = frozenset((
		...
		base_types.FieldEntry(name='OthrId', type=RestrictedFINXMax16Text, min=0, max=1, mutex_group=None, array=False),
