...
from . import DateType10Code
from . import GenericIdentification30

class DateCode34Choice(base_types._BaseFieldType):
	...
	@Cd.setter
	def Cd(self, value):
		self._Cd = value if value is not None else base_types.UninitialisedField(self, 'Cd', DateType10Code, False)

	@Cd.deleter
	def Cd(self):
		del self._Cd
		self._Cd = base_types.UninitialisedField(self, 'Cd', DateType10Code, False)
	...
	_field_defs = frozenset((
		base_types.FieldEntry(name='Cd', type=DateType10Code, min=0, max=1, mutex_group=1, array=False),
