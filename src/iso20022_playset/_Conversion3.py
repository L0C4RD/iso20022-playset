from . import base_types
from ._FinancialInstrumentIdentification5 import FinancialInstrumentIdentification5
from ._AdditionalInformation15 import AdditionalInformation15

class Conversion3(base_types._BaseFieldType):

	__slots__ = ["_AddtlInf", "_OrgnlScty"]
	@property
	def AddtlInf(self):
		return self._AddtlInf

	@AddtlInf.setter
	def AddtlInf(self, value):
		self._AddtlInf = value if type(value) != base_types.auto else self.make_default("AddtlInf")

	@AddtlInf.deleter
	def AddtlInf(self):
		del self._AddtlInf
		self._AddtlInf = None

	@property
	def OrgnlScty(self):
		return self._OrgnlScty

	@OrgnlScty.setter
	def OrgnlScty(self, value):
		self._OrgnlScty = value if type(value) != base_types.auto else self.make_default("OrgnlScty")

	@OrgnlScty.deleter
	def OrgnlScty(self):
		del self._OrgnlScty
		self._OrgnlScty = None

	_field_defs = frozenset((
		base_types.FieldEntry(name='AddtlInf', type=AdditionalInformation15, min=0, max=None, mutex_group=None, array=True),
		base_types.FieldEntry(name='OrgnlScty', type=FinancialInstrumentIdentification5, min=1, max=1, mutex_group=None, array=False),
	))

