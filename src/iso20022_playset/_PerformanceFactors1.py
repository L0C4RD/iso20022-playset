...
class PerformanceFactors1(base_types._BaseFieldType):
	...
	def AcmltnPrd(self, value):
		self._AcmltnPrd = value if value is not None else base_types.UninitialisedField(self, 'AcmltnPrd', DatePeriodDetails, False)

	@AcmltnPrd.deleter
	def AcmltnPrd(self):
		del self._AcmltnPrd
		self._AcmltnPrd = base_types.UninitialisedField(self, 'AcmltnPrd', DatePeriodDetails, False)
