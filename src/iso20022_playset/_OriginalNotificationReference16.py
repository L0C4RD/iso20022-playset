...
from . import ISODate
from . import OriginalItem9
from . import Party50Choice

class OriginalNotificationReference16(base_types._BaseFieldType):

	__slots__ = ["_Acct", "_AcctOwnr", "_AcctSvcr", "_Dbtr", "_DbtrAgt", "_IntrmyAgt", "_OrgnlItm", "_RltdAcct", "_TtlAmt", "_XpctdValDt"]
	@property
	def Acct(self):
		return self._Acct
	...
	def OrgnlItm(self, value):
		self._OrgnlItm = value if value is not None else base_types.UninitialisedField(self, 'OrgnlItm', OriginalItem9, True)

	@OrgnlItm.deleter
	def OrgnlItm(self):
		del self._OrgnlItm
		self._OrgnlItm = base_types.UninitialisedField(self, 'OrgnlItm', OriginalItem9, True)
	...
	_field_defs = frozenset((
		...
		base_types.FieldEntry(name='OrgnlItm', type=OriginalItem9, min=1, max=None, mutex_group=None, array=True),
