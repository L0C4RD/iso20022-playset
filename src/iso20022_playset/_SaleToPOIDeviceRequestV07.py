...
class SaleToPOIDeviceRequestV07(base_types._BaseFieldType):
	...
	def DvcReq(self, value):
		self._DvcReq = value if value is not None else base_types.UninitialisedField(self, 'DvcReq', DeviceRequest8, False)

	@DvcReq.deleter
	def DvcReq(self):
		del self._DvcReq
		self._DvcReq = base_types.UninitialisedField(self, 'DvcReq', DeviceRequest8, False)
