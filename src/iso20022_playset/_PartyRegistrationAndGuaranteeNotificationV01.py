...
class PartyRegistrationAndGuaranteeNotificationV01(base_types._BaseFieldType):
	...
	__slots__ = ["_AttchdMsg", "_CtrlSum", "_Hdr", "_ItmCnt", "_NtfctnCnt", "_NtfctnList"]
	@property
	def AttchdMsg(self):
	...
	def NtfctnCnt(self, value):
		self._NtfctnCnt = value if value is not None else base_types.UninitialisedField(self, 'NtfctnCnt', Max15NumericText, False)

	@NtfctnCnt.deleter
	def NtfctnCnt(self):
		del self._NtfctnCnt
		self._NtfctnCnt = base_types.UninitialisedField(self, 'NtfctnCnt', Max15NumericText, False)
	...
	def NtfctnList(self, value):
		self._NtfctnList = value if value is not None else base_types.UninitialisedField(self, 'NtfctnList', FinancingAgreementList1, True)

	@NtfctnList.deleter
	def NtfctnList(self):
		del self._NtfctnList
		self._NtfctnList = base_types.UninitialisedField(self, 'NtfctnList', FinancingAgreementList1, True)
