...
class ContentInformationType10(base_types._BaseFieldType):
	...
	def EnvlpdData(self, value):
		self._EnvlpdData = value if value is not None else base_types.UninitialisedField(self, 'EnvlpdData', EnvelopedData4, False)

	@EnvlpdData.deleter
	def EnvlpdData(self):
		del self._EnvlpdData
		self._EnvlpdData = base_types.UninitialisedField(self, 'EnvlpdData', EnvelopedData4, False)
