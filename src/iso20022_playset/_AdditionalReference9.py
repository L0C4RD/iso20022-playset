...
class AdditionalReference9(base_types._BaseFieldType):
	...
	def RefIssr(self, value):
		self._RefIssr = value if value is not None else base_types.UninitialisedField(self, 'RefIssr', PartyIdentification113, False)

	@RefIssr.deleter
	def RefIssr(self):
		del self._RefIssr
		self._RefIssr = base_types.UninitialisedField(self, 'RefIssr', PartyIdentification113, False)
