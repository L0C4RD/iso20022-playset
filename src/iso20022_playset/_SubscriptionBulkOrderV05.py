...
from . import SubscriptionBulkOrder06

class SubscriptionBulkOrderV05(base_types._BaseFieldType):
	...
	def BlkOrdrDtls(self, value):
		self._BlkOrdrDtls = value if value is not None else base_types.UninitialisedField(self, 'BlkOrdrDtls', SubscriptionBulkOrder06, False)

	@BlkOrdrDtls.deleter
	def BlkOrdrDtls(self):
		del self._BlkOrdrDtls
		self._BlkOrdrDtls = base_types.UninitialisedField(self, 'BlkOrdrDtls', SubscriptionBulkOrder06, False)
	...
	_field_defs = frozenset((
		base_types.FieldEntry(name='BlkOrdrDtls', type=SubscriptionBulkOrder06, min=1, max=1, mutex_group=None, array=False),
