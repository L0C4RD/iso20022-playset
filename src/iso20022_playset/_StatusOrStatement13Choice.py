...
class StatusOrStatement13Choice(base_types._BaseFieldType):
	...
	def StsAdvc(self, value):
		self._StsAdvc = value if value is not None else base_types.UninitialisedField(self, 'StsAdvc', DocumentNumber21, False)

	@StsAdvc.deleter
	def StsAdvc(self):
		del self._StsAdvc
		self._StsAdvc = base_types.UninitialisedField(self, 'StsAdvc', DocumentNumber21, False)
