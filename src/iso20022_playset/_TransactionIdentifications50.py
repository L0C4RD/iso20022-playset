...
from . import References59Choice
from . import RestrictedFINXMax16Text

class TransactionIdentifications50(base_types._BaseFieldType):

	__slots__ = ["_AcctOwnrTxId", "_AcctSvcrTxId", "_CtrPtyMktInfrstrctrTxId", "_MktInfrstrctrTxId", "_PrcrTxId"]
	@property
	...
	def AcctOwnrTxId(self, value):
		self._AcctOwnrTxId = value if value is not None else base_types.UninitialisedField(self, 'AcctOwnrTxId', References59Choice, False)

	@AcctOwnrTxId.deleter
	def AcctOwnrTxId(self):
		del self._AcctOwnrTxId
		self._AcctOwnrTxId = base_types.UninitialisedField(self, 'AcctOwnrTxId', References59Choice, False)
	...
	_field_defs = frozenset((
		base_types.FieldEntry(name='AcctOwnrTxId', type=References59Choice, min=1, max=1, mutex_group=None, array=False),
