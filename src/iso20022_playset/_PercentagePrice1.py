...
class PercentagePrice1(base_types._BaseFieldType):
	...
	def PricVal(self, value):
		self._PricVal = value if value is not None else base_types.UninitialisedField(self, 'PricVal', PercentageRate, False)

	@PricVal.deleter
	def PricVal(self):
		del self._PricVal
		self._PricVal = base_types.UninitialisedField(self, 'PricVal', PercentageRate, False)

	_field_defs = frozenset((
		base_types.FieldEntry(name='PctgPricTp', type=PriceRateType3Code, min=1, max=1, mutex_group=None, array=False),
		base_types.FieldEntry(name='PricVal', type=PercentageRate, min=1, max=1, mutex_group=None, array=False),
	))