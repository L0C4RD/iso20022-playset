...
class PartyIdentification273(base_types._BaseFieldType):
	...
	def CtctDtls(self, value):
		self._CtctDtls = value if value is not None else base_types.UninitialisedField(self, 'CtctDtls', Contact13, False)

	@CtctDtls.deleter
	def CtctDtls(self):
		del self._CtctDtls
		self._CtctDtls = base_types.UninitialisedField(self, 'CtctDtls', Contact13, False)
	...
	@Id.setter
	def Id(self, value):
		self._Id = value if value is not None else base_types.UninitialisedField(self, 'Id', Party56Choice, False)

	@Id.deleter
	def Id(self):
		del self._Id
		self._Id = base_types.UninitialisedField(self, 'Id', Party56Choice, False)
	...
	def LglNm(self, value):
		self._LglNm = value if value is not None else base_types.UninitialisedField(self, 'LglNm', Max140Text, False)

	@LglNm.deleter
	def LglNm(self):
		del self._LglNm
		self._LglNm = base_types.UninitialisedField(self, 'LglNm', Max140Text, False)
