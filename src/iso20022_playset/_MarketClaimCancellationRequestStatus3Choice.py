...
from . import PendingCancellationStatus22Choice
from . import ProprietaryStatusAndReason6
from . import RejectedStatus38Choice

class MarketClaimCancellationRequestStatus3Choice(base_types._BaseFieldType):
	...
	def PdgCxl(self, value):
		self._PdgCxl = value if value is not None else base_types.UninitialisedField(self, 'PdgCxl', PendingCancellationStatus22Choice, False)

	@PdgCxl.deleter
	def PdgCxl(self):
		del self._PdgCxl
		self._PdgCxl = base_types.UninitialisedField(self, 'PdgCxl', PendingCancellationStatus22Choice, False)
	...
	_field_defs = frozenset((
		...
		base_types.FieldEntry(name='PdgCxl', type=PendingCancellationStatus22Choice, min=0, max=1, mutex_group=1, array=False),
