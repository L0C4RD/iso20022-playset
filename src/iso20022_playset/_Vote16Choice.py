...
class Vote16Choice(base_types._BaseFieldType):
	...
	def VoteInstr(self, value):
		self._VoteInstr = value if value is not None else base_types.UninitialisedField(self, 'VoteInstr', Vote14, True)

	@VoteInstr.deleter
	def VoteInstr(self):
		del self._VoteInstr
		self._VoteInstr = base_types.UninitialisedField(self, 'VoteInstr', Vote14, True)
