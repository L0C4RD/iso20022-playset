...
class KEKIdentifier7(base_types._BaseFieldType):
	...
	def DerivtnId(self, value):
		self._DerivtnId = value if value is not None else base_types.UninitialisedField(self, 'DerivtnId', Max500Binary, False)

	@DerivtnId.deleter
	def DerivtnId(self):
		del self._DerivtnId
		self._DerivtnId = base_types.UninitialisedField(self, 'DerivtnId', Max500Binary, False)
