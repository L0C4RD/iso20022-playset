...
from . import EventFrequency7Code
from . import GenericIdentification30

class Frequency22Choice(base_types._BaseFieldType):
	...
	@Cd.setter
	def Cd(self, value):
		self._Cd = value if value is not None else base_types.UninitialisedField(self, 'Cd', EventFrequency7Code, False)

	@Cd.deleter
	def Cd(self):
		del self._Cd
		self._Cd = base_types.UninitialisedField(self, 'Cd', EventFrequency7Code, False)
	...
	_field_defs = frozenset((
		base_types.FieldEntry(name='Cd', type=EventFrequency7Code, min=0, max=1, mutex_group=1, array=False),
