...
from . import SecuritiesBalanceCustodyReportV12
...
class SEMT_002_001_12():
	...
	class Document(base_types._BaseFieldType_Document):
		...
		def SctiesBalCtdyRpt(self, value):
			self._SctiesBalCtdyRpt = value if value is not None else base_types.UninitialisedField(self, 'SctiesBalCtdyRpt', SecuritiesBalanceCustodyReportV12, False)

		@SctiesBalCtdyRpt.deleter
		def SctiesBalCtdyRpt(self):
			del self._SctiesBalCtdyRpt
			self._SctiesBalCtdyRpt = base_types.UninitialisedField(self, 'SctiesBalCtdyRpt', SecuritiesBalanceCustodyReportV12, False)
