...
class AcknowledgementV03(base_types._BaseFieldType):
	...
	__slots__ = ["_AckId", "_AckdMsgRef", "_EstblishdBaselnId", "_ReqForActn", "_TxId", "_TxSts", "_UsrTxRef"]
	@property
	def AckId(self):
		return self._AckId
	...
	def AckdMsgRef(self, value):
		self._AckdMsgRef = value if value is not None else base_types.UninitialisedField(self, 'AckdMsgRef', MessageIdentification1, False)

	@AckdMsgRef.deleter
	def AckdMsgRef(self):
		del self._AckdMsgRef
		self._AckdMsgRef = base_types.UninitialisedField(self, 'AckdMsgRef', MessageIdentification1, False)
