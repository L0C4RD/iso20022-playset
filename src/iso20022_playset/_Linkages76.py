...
from . import IdentificationReference15Choice
...
class Linkages76(base_types._BaseFieldType):
	...
	@Ref.setter
	def Ref(self, value):
		self._Ref = value if value is not None else base_types.UninitialisedField(self, 'Ref', IdentificationReference15Choice, False)

	@Ref.deleter
	def Ref(self):
		del self._Ref
		self._Ref = base_types.UninitialisedField(self, 'Ref', IdentificationReference15Choice, False)

	_field_defs = frozenset((
		base_types.FieldEntry(name='MsgNb', type=DocumentNumber17Choice, min=0, max=1, mutex_group=None, array=False),
		base_types.FieldEntry(name='Ref', type=IdentificationReference15Choice, min=1, max=1, mutex_group=None, array=False),
	))