...
from . import ExpectedExecutionDetails2
from . import FundOrderData6
from . import Max35Text
from . import OrderStatus4Choice
from . import PartyIdentification113
from . import SwitchLegReferences2
...
class SwitchOrderStatusAndReason2(base_types._BaseFieldType):
	...
	def LegInf(self, value):
		self._LegInf = value if value is not None else base_types.UninitialisedField(self, 'LegInf', SwitchLegReferences2, True)

	@LegInf.deleter
	def LegInf(self):
		del self._LegInf
		self._LegInf = base_types.UninitialisedField(self, 'LegInf', SwitchLegReferences2, True)
	...
	def NewDtls(self, value):
		self._NewDtls = value if value is not None else base_types.UninitialisedField(self, 'NewDtls', ExpectedExecutionDetails2, False)

	@NewDtls.deleter
	def NewDtls(self):
		del self._NewDtls
		self._NewDtls = base_types.UninitialisedField(self, 'NewDtls', ExpectedExecutionDetails2, False)
	...
	def OrdrSts(self, value):
		self._OrdrSts = value if value is not None else base_types.UninitialisedField(self, 'OrdrSts', OrderStatus4Choice, False)

	@OrdrSts.deleter
	def OrdrSts(self):
		del self._OrdrSts
		self._OrdrSts = base_types.UninitialisedField(self, 'OrdrSts', OrderStatus4Choice, False)
