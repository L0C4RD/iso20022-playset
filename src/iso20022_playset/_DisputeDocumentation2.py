...
from . import OutputFormat6Code

class DisputeDocumentation2(base_types._BaseFieldType):

	__slots__ = ["_Frmt", "_Tp", "_Val"]
	@property
	def Frmt(self):
		return self._Frmt
	...
	@Frmt.setter
	def Frmt(self, value):
		self._Frmt = value if value is not None else base_types.UninitialisedField(self, 'Frmt', OutputFormat6Code, False)

	@Frmt.deleter
	def Frmt(self):
		del self._Frmt
		self._Frmt = base_types.UninitialisedField(self, 'Frmt', OutputFormat6Code, False)
	...
	_field_defs = frozenset((
		base_types.FieldEntry(name='Frmt', type=OutputFormat6Code, min=0, max=1, mutex_group=None, array=False),
