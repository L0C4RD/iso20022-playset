...
class AccountIdentification74(base_types._BaseFieldType):
	...
	def CorpActnEvtAndBal(self, value):
		self._CorpActnEvtAndBal = value if value is not None else base_types.UninitialisedField(self, 'CorpActnEvtAndBal', CorporateActionEventAndBalance27, True)

	@CorpActnEvtAndBal.deleter
	def CorpActnEvtAndBal(self):
		del self._CorpActnEvtAndBal
		self._CorpActnEvtAndBal = base_types.UninitialisedField(self, 'CorpActnEvtAndBal', CorporateActionEventAndBalance27, True)
	...
	def SfkpgPlc(self, value):
		self._SfkpgPlc = value if value is not None else base_types.UninitialisedField(self, 'SfkpgPlc', SafekeepingPlaceFormat46Choice, False)

	@SfkpgPlc.deleter
	def SfkpgPlc(self):
		del self._SfkpgPlc
		self._SfkpgPlc = base_types.UninitialisedField(self, 'SfkpgPlc', SafekeepingPlaceFormat46Choice, False)
