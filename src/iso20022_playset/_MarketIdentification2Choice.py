...
class MarketIdentification2Choice(base_types._BaseFieldType):
	...
	@Desc.setter
	def Desc(self, value):
		self._Desc = value if value is not None else base_types.UninitialisedField(self, 'Desc', RestrictedFINXMax30Text, False)

	@Desc.deleter
	def Desc(self):
		del self._Desc
		self._Desc = base_types.UninitialisedField(self, 'Desc', RestrictedFINXMax30Text, False)
