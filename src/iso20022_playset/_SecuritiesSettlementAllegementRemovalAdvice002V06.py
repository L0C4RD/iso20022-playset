...
from . import TransactionDetails156
...
class SecuritiesSettlementAllegementRemovalAdvice002V06(base_types._BaseFieldType):
	...
	def AcctSvcrTxId(self, value):
		self._AcctSvcrTxId = value if value is not None else base_types.UninitialisedField(self, 'AcctSvcrTxId', SettlementTypeAndIdentification22, False)

	@AcctSvcrTxId.deleter
	def AcctSvcrTxId(self):
		del self._AcctSvcrTxId
		self._AcctSvcrTxId = base_types.UninitialisedField(self, 'AcctSvcrTxId', SettlementTypeAndIdentification22, False)
	...
	def TxDtls(self, value):
		self._TxDtls = value if value is not None else base_types.UninitialisedField(self, 'TxDtls', TransactionDetails156, False)

	@TxDtls.deleter
	def TxDtls(self):
		del self._TxDtls
		self._TxDtls = base_types.UninitialisedField(self, 'TxDtls', TransactionDetails156, False)

	_field_defs = frozenset((
		base_types.FieldEntry(name='AcctOwnr', type=PartyIdentification156, min=0, max=1, mutex_group=None, array=False),
		base_types.FieldEntry(name='AcctSvcrTxId', type=SettlementTypeAndIdentification22, min=1, max=1, mutex_group=None, array=False),
		...
		base_types.FieldEntry(name='TxDtls', type=TransactionDetails156, min=0, max=1, mutex_group=None, array=False),
	))