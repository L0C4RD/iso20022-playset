...
from . import CalendarReportOrError1Choice
...
class CalendarReportV02(base_types._BaseFieldType):
	...
	def RptOrErr(self, value):
		self._RptOrErr = value if value is not None else base_types.UninitialisedField(self, 'RptOrErr', CalendarReportOrError1Choice, False)

	@RptOrErr.deleter
	def RptOrErr(self):
		del self._RptOrErr
		self._RptOrErr = base_types.UninitialisedField(self, 'RptOrErr', CalendarReportOrError1Choice, False)
	...
	_field_defs = frozenset((
		...
		base_types.FieldEntry(name='RptOrErr', type=CalendarReportOrError1Choice, min=1, max=1, mutex_group=None, array=False),
