...
from . import SubscriptionBulkOrder5

class SubscriptionBulkOrderV04(base_types._BaseFieldType):
	...
	def BlkOrdrDtls(self, value):
		self._BlkOrdrDtls = value if value is not None else base_types.UninitialisedField(self, 'BlkOrdrDtls', SubscriptionBulkOrder5, False)

	@BlkOrdrDtls.deleter
	def BlkOrdrDtls(self):
		del self._BlkOrdrDtls
		self._BlkOrdrDtls = base_types.UninitialisedField(self, 'BlkOrdrDtls', SubscriptionBulkOrder5, False)
	...
	_field_defs = frozenset((
		base_types.FieldEntry(name='BlkOrdrDtls', type=SubscriptionBulkOrder5, min=1, max=1, mutex_group=None, array=False),
