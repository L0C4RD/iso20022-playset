...
class GenericIdentification164(base_types._BaseFieldType):
	...
	@IdTp.setter
	def IdTp(self, value):
		self._IdTp = value if value is not None else base_types.UninitialisedField(self, 'IdTp', OtherIdentification3Choice, False)

	@IdTp.deleter
	def IdTp(self):
		del self._IdTp
		self._IdTp = base_types.UninitialisedField(self, 'IdTp', OtherIdentification3Choice, False)
